# server_distilbert.py — Federated server for DistilBERT + DistMult experiment (PQ2H)
#
# DistMult entity embeddings are d-dimensional (real), same as TransE.
# h_joint = [h_A || h_B || h_C] ∈ ℝ^(3d) = ℝ^768
# QuestionEncoder MLP output dim = 3d = 768  (same as DistilBERT+TransE)
#
# Only difference from DistilBert+TransE server_distilbert.py:
#   - Imports from config_distilbert_distmult instead of config_distilbert_transe
# Everything else — DistilBERT encoder, fusion, scoring, loss — is identical.
...
class FedVServer(nn.Module):
    """
    Central server for DistilBERT + DistMult FedV-KGQA (3 silos, PQ2H).
