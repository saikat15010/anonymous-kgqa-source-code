# train_kge.py — Train TransE independently on each of the 3 silos (PQ2H)
...
class KGELogger:
    ...
    Writes KGE training results to:
      logs/run_<id>/kge_train_log.txt
      logs/run_<id>/kge_train_log.json
    """
    ...
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            f.write("=" * 70 + "\n")
            f.write("  FedV-KGQA  —  KGE Training Log  [TransE | PQ2H]\n")
...
def train_silo(silo_name, kb_path, shared_entity2id, device, logger):
    print(f"\n{'='*60}")
    print(f"  Training TransE — {silo_name}")
    print(f"{'='*60}")
