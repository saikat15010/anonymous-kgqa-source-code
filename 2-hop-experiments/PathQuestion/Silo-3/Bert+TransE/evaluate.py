# evaluate.py — Evaluation: MRR, Hits@1/3/5/10 (overall + per answer type)
# PQ2H version — answer types reflect Freebase13 relations across 3 silos:
#   Silo A: children, parents, spouse
#   Silo B: cause_of_death, ethnicity, gender, nationality, religion
#   Silo C: institution, location, place_of_birth, place_of_death, profession
...
# ── Answer type detection from question string ────────────────────────────────

def detect_answer_type(question):
    """
    Infer expected answer type from question keywords.
    PQ2H questions are 2-hop; we detect the second-hop relation type
    (the type of the final answer entity) from surface patterns.

    Returns one of:
        'family'     — children / parents / spouse questions
        'gender'     — gender / sex questions
        'nationality'— nationality / citizenship questions
        'ethnicity'  — ethnicity / race questions
        'religion'   — religion / faith questions
        'cause_death'— cause of death questions
        'place'      — place_of_birth / place_of_death / location questions
        'institution'— institution / organization / school questions
        'profession' — profession / job / occupation questions
        'unknown'    — unrecognised
    """
    q = question.lower()

    # Family relations (Silo A)
    ...
    # Biographical relations (Silo C)
    if any(w in q for w in ["born", "birth", "birthplace",
                             "place of birth", "hometown"]):
        return "place"
    if any(w in q for w in ["place of death", "die", "died",
                             "death", "buried"]):
        return "place"
...
def evaluate(server, model_a, model_b, model_c, qa_loader, device,
             per_type=True):
    """
    Evaluate over precomputed candidate sets.

    Returns:
        overall_results  : dict of MRR + Hits metrics
        per_type_results : dict of answer_type -> metric dict
    """
    server.eval()
