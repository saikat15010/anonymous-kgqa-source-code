# evaluate_roberta_complex.py — Evaluation for RoBERTa + ComplEx experiment (PQ2H)
# Identical logic to all other PQ2H evaluate files.
# Imports USE_TOPIC_ANCHORING from config_roberta_complex.
...
def evaluate(server, model_a, model_b, model_c, qa_loader, device,
    ...
    with torch.no_grad():
        ...
        h_joint = server.fuse(h_a, h_b, h_c)   # (N, 6d)
...
def print_results(split_name, overall, per_type=None):
    ...
    print(f"\n{'─'*70}")
    print(f"  {split_name} Results  [RoBERTa + ComplEx | PQ2H]")
    print(f"{'─'*70}")
