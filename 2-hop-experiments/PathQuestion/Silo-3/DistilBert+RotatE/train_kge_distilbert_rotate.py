# train_kge_distilbert_rotate.py — Train RotatE independently on each of the 3 silos (PQ2H)
# KGE training is completely independent of the QA encoder choice.
# All results saved to logs_distilbert_rotate/run_<timestamp>/kge_train_log.{txt,json}
...
class KGELogger:
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            f.write("=" * 70 + "\n")
            f.write("  FedV-KGQA  —  KGE Training Log  [RotatE + DistilBERT | PQ2H]\n")
...
def train_silo(silo_name, kb_path, shared_entity2id, device, logger):
    print(f"\n{'='*60}")
    print(f"  Training RotatE — {silo_name}  [for DistilBERT+RotatE | PQ2H]")
    print(f"{'='*60}")
    ...
    for epoch in range(1, KGE_EPOCHS + 1):
        ...
        for h, r, t_pos in loader:
            ...
            # RotatE: normalize entity embeddings
            with torch.no_grad():
