# evaluate_roberta_transe.py — Evaluation for RoBERTa + TransE experiment (PQ2H)
# Identical logic to all other PQ2H evaluate files.
# Imports USE_TOPIC_ANCHORING from config_roberta_transe.
# Answer types reflect Freebase13 relations across 3 silos (PQ2H).
...
def detect_answer_type(question):
    ...
    PQ2H questions are 2-hop over Freebase13 relations.
    ...
    Returns one of:
        ...
        'place'       — place_of_birth / place_of_death / location
        'institution' — institution / organization / school
...
def print_results(split_name, overall, per_type=None):
    ...
    print(f"\n{'─'*70}")
    print(f"  {split_name} Results  [RoBERTa + TransE | PQ2H]")
    print(f"{'─'*70}")
