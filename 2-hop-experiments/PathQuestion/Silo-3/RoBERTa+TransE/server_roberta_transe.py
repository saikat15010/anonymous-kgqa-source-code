# server_roberta_transe.py — Federated server for RoBERTa + TransE experiment (PQ2H)
#
# Differences from Bert+TransE server.py:
#   - RobertaTokenizer + RobertaModel instead of BertTokenizer + BertModel
#   - RoBERTa does NOT use token_type_ids — tokenizer handles this automatically
#   - <s> token serves as sequence representation (equivalent to BERT's [CLS])
#   - Imports from config_roberta_transe
#
# TransE entity embeddings are d-dimensional (real), same as BERT+TransE.
# h_joint = [h_A || h_B || h_C] ∈ ℝ^(3d) = ℝ^768 — UNCHANGED.
# Everything else — fusion, scoring, loss, topic anchoring — is identical.
...
class QuestionEncoder(nn.Module):
    ...
    RoBERTa (Robustly Optimised BERT Pre-training Approach):
      ...
      - Typically outperforms BERT-base on downstream NLP benchmarks

    Key implementation note:
      - RoBERTa tokenizer does NOT produce token_type_ids
      - First token <s> (BOS) is the sequence representation,
    ...
    def __init__(self):
        ...
        # Freeze RoBERTa — only MLP trains during federated QA
        for param in self.roberta.parameters():
            param.requires_grad = False

        # MLP: 768 → 512 → 768  (identical dims to BERT and DistilBERT versions)
    ...
    def forward(self, questions, device):
        ...
        # <s> token representation — equivalent to BERT's [CLS]
...
class FedVServer(nn.Module):
    """
    Central server for RoBERTa + TransE FedV-KGQA (3 silos, PQ2H).
    h_joint(e) = [h_A(e) || h_B(e) || h_C(e)] ∈ ℝ^(3d)

    TransE entity embeddings are d-dimensional (real),
    so joint dim = 3d = 768. Identical to BERT+TransE server
    except the encoder is RoBERTa.
    """
