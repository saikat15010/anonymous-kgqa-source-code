# server_distilbert.py — Federated server for DistilBERT + ComplEx experiment (PQ2H)
#
...
#
# Structurally identical to DistilBert+RotatE server_distilbert.py.
# Only difference: imports from config_distilbert_complex instead of
# config_distilbert_rotate. Both ComplEx and RotatE use 2d entity embeddings,
# so joint_dim = 6d = 1536 in both cases.
...
# ComplEx stores 2*embed_dim reals per entity (real + imaginary parts)
...
class QuestionEncoder(nn.Module):
    """
    DistilBERT [CLS] → MLP → q_embed ∈ ℝ^(6d)
...
class FedVServer(nn.Module):
    """
    Central server for DistilBERT + ComplEx FedV-KGQA (3 silos, PQ2H).
