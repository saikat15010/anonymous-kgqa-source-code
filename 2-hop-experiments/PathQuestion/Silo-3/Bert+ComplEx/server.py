# server.py — Federated server for ComplEx experiment (PQ2H)
#
# ComplEx entity embeddings are 2*embed_dim (complex representation).
# h_joint = [h_A || h_B || h_C] ∈ ℝ^(3 * 2d) = ℝ^(6d)
# QuestionEncoder MLP output dim = 6d  (same as RotatE server)
#
# Structurally identical to RotatE server — only config import differs.
...
class FedVServer(nn.Module):
    """
    Central server for ComplEx FedV-KGQA (3 silos, PQ2H).
