# train_kge_roberta_distmult.py — Train DistMult independently on each of the 3 silos (PQ2H)
# KGE training is completely independent of the QA encoder choice.
# All results saved to logs_roberta_distmult/run_<timestamp>/kge_train_log.{txt,json}
...
class KGELogger:
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            f.write("=" * 70 + "\n")
            f.write("  FedV-KGQA  —  KGE Training Log  [DistMult + RoBERTa | PQ2H]\n")
...
def train_silo(silo_name, kb_path, shared_entity2id, device, logger):
    print(f"\n{'='*60}")
    print(f"  Training DistMult — {silo_name}  [for RoBERTa+DistMult | PQ2H]")
    print(f"{'='*60}")
