# train_fedv.py — Federated QA training ComplEx + DistilBERT (PQ2H | Client7 — 7 silos)
# QA-phase optimizers use NO weight decay (weight_decay=1e-6 is KGE-phase only).
...
class FedVLogger:
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            ...
            f.write("  FedV-KGQA  —  Federated QA Training Log"
                    "  [ComplEx + DistilBERT | PQ2H | Client7 — 7 silos]\n")
    ...
    def log_dev(self, epoch, train_avg_loss, overall, per_type):
        ...
        with open(self.txt_path, "a") as f:
            f.write("\n" + "-" * 70 + "\n")
            f.write(f"  DEV  Epoch {epoch} | Loss {train_avg_loss:.6f}\n")
            f.write("-" * 70 + "\n")
...
def load_complex(silo_name, device):
    ckpt = torch.load(
        ...
        map_location=device)
    model = ComplEx(num_entities=ckpt["num_entities"],
...
def train():
    ...
    print(f"Client7 — 7 silos | entity_dim=2x{KGE_EMBED_DIM}={2*KGE_EMBED_DIM}"
          f" | joint_dim=7x{2*KGE_EMBED_DIM}={7*2*KGE_EMBED_DIM}")
    ...
    print("\nLoading pretrained ComplEx models (7 silos)...")
    model_a = load_complex("silo_a", device)
    model_b = load_complex("silo_b", device)
    model_c = load_complex("silo_c", device)
    model_d = load_complex("silo_d", device)
    model_e = load_complex("silo_e", device)
    model_f = load_complex("silo_f", device)
    model_g = load_complex("silo_g", device)
    ...
    # QA-phase: no weight_decay (weight_decay=1e-6 is KGE-phase only)
    ...
    for epoch in range(1, QA_EPOCHS + 1):
        ...
        for batch_idx, (questions, topic_ids,
            ...
            for opt in [optim_a, optim_b, optim_c, optim_d,
                        optim_e, optim_f, optim_g]:
                opt.zero_grad()

            loss, _ = server(
                questions, topic_ids,
                model_a.ent_embed.weight, model_b.ent_embed.weight,
                model_c.ent_embed.weight, model_d.ent_embed.weight,
                model_e.ent_embed.weight, model_f.ent_embed.weight,
                model_g.ent_embed.weight,
            ...
            # ComplEx: L2-normalize entity embeddings in all 7 silos
            with torch.no_grad():
                for m in [model_a, model_b, model_c, model_d,
                          model_e, model_f, model_g]:
    ...
    print("\n" + "=" * 60)
    print("  FINAL TEST RESULTS (ComplEx+DistilBERT | PQ2H | Client7 | 7 silos)")
    print("=" * 60)
...
def _save(server, ma, mb, mc, md, me, mf, mg, tag):
    ...
    torch.save({
        ...
        "model_e": me.state_dict(), "model_f": mf.state_dict(),
...
def _load(server, ma, mb, mc, md, me, mf, mg, tag, device):
    ...
    ma.load_state_dict(ckpt["model_a"]); mb.load_state_dict(ckpt["model_b"])
    mc.load_state_dict(ckpt["model_c"]); md.load_state_dict(ckpt["model_d"])
    me.load_state_dict(ckpt["model_e"]); mf.load_state_dict(ckpt["model_f"])
