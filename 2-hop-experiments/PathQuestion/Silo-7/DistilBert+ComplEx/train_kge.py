# train_kge.py — Train ComplEx independently on each of the 7 silos (PQ2H | Client7)
# KGE training is encoder-agnostic — DistilBERT is not involved here.
# ComplEx: Adam optimizer uses weight_decay=1e-6 (standard practice, unique to ComplEx).
    ...
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            ...
            f.write("  FedV-KGQA  —  KGE Training Log"
                    "  [ComplEx | PQ2H | Client7 — 7 silos | DistilBERT]\n")
...
def train_silo(silo_name, kb_path, shared_entity2id, device, logger):
    print(f"\n{'='*60}")
    print(f"  Training ComplEx — {silo_name}  [Client7 | DistilBERT+ComplEx]")
    print(f"{'='*60}")
    ...
    # ComplEx: weight_decay=1e-6 (standard practice, unique to ComplEx)
...
def main():
    ...
    shared_entity2id = build_shared_entity_index(
        e2id_a, e2id_b, e2id_c, e2id_d,
        e2id_e, e2id_f, e2id_g)
