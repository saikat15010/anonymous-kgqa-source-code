# server.py — Federated server for Client7 RoBERTa+DistMult (7 silos, PQ2H)
#
...
#   - MLP: 768 -> 512 -> 1792
#
# Structurally identical to Client7/RoBERTa+TransE server.py.
# Only config import points to RoBERTa+DistMult paths.
...
class QuestionEncoder(nn.Module):
    ...
    DistMult uses d-dim real entity embeddings -> joint_dim = 7d = 1792.
    MLP architecture identical to RoBERTa+TransE.
    Critical RoBERTa note: no token_type_ids.
    """
...
class FedVServer(nn.Module):
    """
    Central server — Client7 RoBERTa+DistMult (7 silos).
