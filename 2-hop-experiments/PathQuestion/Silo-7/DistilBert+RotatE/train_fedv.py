# train_fedv.py — Federated QA training RotatE + DistilBERT (PQ2H | Client7 — 7 silos)
    ...
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            ...
            f.write("  FedV-KGQA  —  Federated QA Training Log"
                    "  [RotatE + DistilBERT | PQ2H | Client7 — 7 silos]\n")
...
def load_rotate(silo_name, device):
    ckpt = torch.load(
        ...
        map_location=device)
    model = RotatE(num_entities=ckpt["num_entities"],
...
def train():
    ...
    print("\nLoading pretrained RotatE models (7 silos)...")
    ...
    model_f = load_rotate("silo_f", device)
    model_g = load_rotate("silo_g", device)
    ...
    for epoch in range(1, QA_EPOCHS + 1):
        ...
        for batch_idx, (questions, topic_ids,
            ...
            # RotatE: normalize entity embeddings in all 7 silos
            with torch.no_grad():
                for m in [model_a, model_b, model_c, model_d,
                          model_e, model_f, model_g]:
    ...
    print("\n" + "=" * 60)
    print("  FINAL TEST RESULTS (RotatE+DistilBERT | PQ2H | Client7 | 7 silos)")
    print("=" * 60)
