# dataset.py — Entity/relation indexing + QA/KGE data loaders (PQ2H | Client7)
#
...
def build_shared_entity_index(*entity2id_dicts):
    """Union of entity sets across ALL silos -> shared index. Works for any N."""
    all_entities = set()
...
def build_neighbor_index(kb_paths, shared_entity2id, max_neighbors=100):
    """Bidirectional neighbor index. Accepts list of paths (any number of silos)."""
...
def parse_qa_file(qa_path):
    """Format: question\ttopic_entity\tanswer  (3 columns, tab-separated)"""
    samples = []
