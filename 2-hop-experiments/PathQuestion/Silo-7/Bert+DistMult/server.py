# server.py — Federated server for Client7 Bert+DistMult (7 silos, PQ2H)
#
# DistMult entity embeddings are d-dimensional (real), same as TransE.
# h_joint = [h_A || h_B || h_C || h_D || h_E || h_F || h_G]
#          in R^(7 * d) = R^1792
# MLP: 768 -> 512 -> 1792
# Structurally identical to Client7/Bert+TransE server.py.
...
class QuestionEncoder(nn.Module):
    """BERT [CLS] -> MLP -> q_embed in R^(7d=1792). BERT frozen."""
...
class FedVServer(nn.Module):
    """
    Central server — Client7 Bert+DistMult (7 silos).
