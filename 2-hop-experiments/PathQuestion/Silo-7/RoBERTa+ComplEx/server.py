# server.py — Federated server for Client7 RoBERTa+ComplEx (7 silos, PQ2H)
#
...
#   - MLP: 768 -> 512 -> 3584
#
# Structurally identical to Client7/RoBERTa+RotatE server.py.
# (Both ComplEx and RotatE use 2d entity embeddings -> JOINT_DIM = 3584.)
# Only config import points to RoBERTa+ComplEx paths.
...
class QuestionEncoder(nn.Module):
    """
    RoBERTa <s> -> MLP -> q_embed in R^(14d=3584). RoBERTa frozen.

    RoBERTa output = 768-dim (same as BERT-base).
    ComplEx uses 2d-dim entity embeddings -> joint_dim = 14d = 3584.
    MLP: 768 -> 512 -> 3584 (identical to RoBERTa+RotatE).
...
class FedVServer(nn.Module):
    """
    Central server — Client7 RoBERTa+ComplEx (7 silos).
