# train_fedv.py — Federated QA training DistMult + DistilBERT (PQ2H | Client7 — 7 silos)
    ...
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            ...
            f.write("  FedV-KGQA  —  Federated QA Training Log"
                    "  [DistMult + DistilBERT | PQ2H | Client7 — 7 silos]\n")
...
def train():
    ...
    print(f"Client7 — 7 silos | joint_dim = 7 x {KGE_EMBED_DIM} = "
          f"{7 * KGE_EMBED_DIM}")
    ...
    print("\nLoading pretrained DistMult models (7 silos)...")
    ...
    model_f = load_distmult("silo_f", device)
    model_g = load_distmult("silo_g", device)
    ...
    for epoch in range(1, QA_EPOCHS + 1):
        ...
        for batch_idx, (questions, topic_ids,
            ...
            # DistMult: L2-normalize entity embeddings in all 7 silos
            with torch.no_grad():
                for m in [model_a, model_b, model_c, model_d,
                          model_e, model_f, model_g]:
    ...
    print("\n" + "=" * 60)
    print("  FINAL TEST RESULTS (DistMult+DistilBERT | PQ2H | Client7 | 7 silos)")
    print("=" * 60)
