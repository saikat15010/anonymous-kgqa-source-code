# train_fedv.py — Federated QA training RotatE + DistilBERT (PQ2H | Client5 — 5 silos)
    ...
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            ...
            f.write("  FedV-KGQA  —  Federated QA Training Log"
                    "  [RotatE + DistilBERT | PQ2H | Client5 — 5 silos]\n")
...
def _fmt_per_type(per_type):
    lines = "  Per Type:\n"
    for atype in ["family","gender","nationality","ethnicity","religion",
                  "cause_death","place","institution","profession","unknown"]:
...
def train():
    ...
    print(f"Client5 — 5 silos | entity_dim=2x{KGE_EMBED_DIM}={2*KGE_EMBED_DIM}"
          f" | joint_dim=5x{2*KGE_EMBED_DIM}={5*2*KGE_EMBED_DIM}")
    ...
    print("\nBuilding neighbor index (5 silos)...")
    ...
    print("\nLoading pretrained RotatE models (5 silos)...")
    ...
    for epoch in range(1, QA_EPOCHS + 1):
        ...
        for batch_idx, (questions, topic_ids,
            ...
            # RotatE: normalize entity embeddings in all 5 silos
            with torch.no_grad():
                for m in [model_a, model_b, model_c, model_d, model_e]:
        ...
        if epoch % 2 == 0 or epoch == QA_EPOCHS:
            ...
            if dev_results["hits@1"] > best_hits1:
                ...
                best_epoch = epoch
                _save(server, model_a, model_b, model_c,
                      model_d, model_e, "best")
    ...
    print("\n" + "=" * 60)
    print("  FINAL TEST RESULTS (RotatE+DistilBERT | PQ2H | Client5 | 5 silos)")
    print("=" * 60)
