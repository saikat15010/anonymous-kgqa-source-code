# server.py — Federated server for Client5 Bert+DistMult (5 silos, PQ2H)
#
# DistMult entity embeddings are d-dimensional (real), same as TransE.
# h_joint = [h_A || h_B || h_C || h_D || h_E] in R^(5 * d) = R^1280
# MLP final layer projects BERT 768-dim output to 1280.
...
class QuestionEncoder(nn.Module):
    """BERT [CLS] -> MLP -> q_embed in R^(5d=1280). BERT frozen."""
...
class FedVServer(nn.Module):
    """
    Central server — Client5 Bert+DistMult (5 silos).
