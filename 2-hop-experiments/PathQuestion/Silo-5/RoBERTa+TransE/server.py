# server.py — Federated server for Client5 RoBERTa+TransE (5 silos, PQ2H)
#
...
#   - MLP: 768 -> 512 -> 1280  (unchanged vs BERT/DistilBERT encoders)
...
class QuestionEncoder(nn.Module):
    """
    RoBERTa <s> -> MLP -> q_embed in R^(5d=1280). RoBERTa frozen.

    RoBERTa output = 768-dim (same as BERT-base).
    TransE uses d-dim real entity embeddings -> joint_dim = 5d = 1280.

    Critical RoBERTa note vs BERT:
      - No token_type_ids — tokenizer does NOT return them; do not pass them
      - <s> token (position 0) = sequence representation, same API as [CLS]
    """
    ...
    def forward(self, questions, device):
        # RoBERTa tokenizer does not produce token_type_ids — pass only
...
class FedVServer(nn.Module):
    """
    Central server — Client5 RoBERTa+TransE (5 silos).
