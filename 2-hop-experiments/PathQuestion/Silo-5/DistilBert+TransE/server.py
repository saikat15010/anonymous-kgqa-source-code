# server.py — Federated server for Client5 DistilBert+TransE (5 silos, PQ2H)
#
# TransE entity embeddings are d-dimensional (real).
# h_joint = [h_A || h_B || h_C || h_D || h_E] in R^(5d) = R^1280
#
# Only difference from Client5/Bert+TransE server.py:
#   - DistilBertTokenizer + DistilBertModel instead of BertTokenizer + BertModel
#   - Uses last_hidden_state[:, 0, :] as CLS token (same API as BERT)
#   - JOINT_DIM = 5d = 1280 (unchanged — DistilBERT output is also 768-dim)
...
class QuestionEncoder(nn.Module):
    ...
    DistilBERT-base:
      ...
      - ~60% faster, ~40% smaller, retains ~97% of BERT NLU performance
    """
...
class FedVServer(nn.Module):
    """
    Central server — Client5 DistilBert+TransE (5 silos).
