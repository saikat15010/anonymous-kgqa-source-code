# train_fedv.py — Federated QA training with TransE + BERT (PQ2H | Client5 — 5 silos)
    ...
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            ...
            f.write("  FedV-KGQA  —  Federated QA Training Log"
                    "  [TransE + BERT | PQ2H | Client5 — 5 silos]\n")
...
def train():
    ...
    print(f"Client5 — 5 silos | joint_dim = 5 × {KGE_EMBED_DIM} = "
          f"{5 * KGE_EMBED_DIM}")
    ...
    # One optimizer per silo entity embedding
    ...
    print(f"\nFederated QA training (TransE + BERT | PQ2H | Client5) — "
    ...
    for epoch in range(1, QA_EPOCHS + 1):
        ...
        for batch_idx, (questions, topic_ids,
            ...
            # L2-normalise entity embeddings in all 5 silos
            with torch.no_grad():
                for m in [model_a, model_b, model_c, model_d, model_e]:
    ...
    _load(server, model_a, model_b, model_c, model_d, model_e,
          "best", device)
