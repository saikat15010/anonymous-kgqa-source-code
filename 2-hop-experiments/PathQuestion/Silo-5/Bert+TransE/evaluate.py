...
# Client5 PQ2H version — 5-silo fuse call.
# Answer types identical to Client3 PQ2H (Freebase13 relations).
...
def evaluate(server, model_a, model_b, model_c, model_d, model_e,
    ...
    """
    Evaluate over precomputed candidate sets — 5-silo version.

    Args:
        model_a … model_e : TransE KGE models (one per silo)
    """
    server.eval()
    ...
    with torch.no_grad():
        ...
        h_joint = server.fuse(h_a, h_b, h_c, h_d, h_e)   # (N, 5d)
...
def print_results(split_name, overall, per_type=None):
    ...
    print(f"\n{'─'*70}")
    print(f"  {split_name} Results  [BERT + TransE | PQ2H | Client5 — 5 silos]")
    print(f"{'─'*70}")
