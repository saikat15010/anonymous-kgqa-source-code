# server.py — Federated server for Client5 Bert+ComplEx (5 silos, PQ2H)
#
# ComplEx entity embeddings are 2*embed_dim (complex representation).
# h_joint = [h_A || h_B || h_C || h_D || h_E] in R^(5 * 2d) = R^2560
# MLP final layer projects BERT 768-dim output to 2560.
...
class QuestionEncoder(nn.Module):
    """BERT [CLS] -> MLP -> q_embed in R^(10d=2560). BERT frozen."""
...
class FedVServer(nn.Module):
    """
    Central server — Client5 Bert+ComplEx (5 silos).
