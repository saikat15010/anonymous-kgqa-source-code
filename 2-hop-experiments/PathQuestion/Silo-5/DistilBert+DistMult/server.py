# server.py — Federated server for Client5 DistilBert+DistMult (5 silos, PQ2H)
#
# DistMult entity embeddings are d-dimensional (real), same as TransE.
# h_joint = [h_A || h_B || h_C || h_D || h_E] in R^(5d) = R^1280
#
# Only difference from Client5/DistilBert+TransE server.py:
#   - Comment updated to say DistMult
#   - Config import reads CHECKPOINT_DIR for DistilBert+DistMult
# JOINT_DIM = 5d = 1280 is unchanged (DistMult also uses d-dim real embeddings).
...
JOINT_DIM = 5 * KGE_EMBED_DIM   # 5 x 256 = 1280


class QuestionEncoder(nn.Module):
    """
    DistilBERT [CLS] -> MLP -> q_embed in R^(5d=1280). DistilBERT frozen.

    DistilBERT output = 768-dim (same as BERT-base).
    DistMult uses d-dim real entity embeddings -> joint_dim = 5d = 1280.
    MLP architecture identical to DistilBert+TransE.
    """
...
class FedVServer(nn.Module):
    """
    Central server — Client5 DistilBert+DistMult (5 silos).
    h_joint(e) = [h_A(e) || h_B(e) || h_C(e) || h_D(e) || h_E(e)] in R^1280
    """
