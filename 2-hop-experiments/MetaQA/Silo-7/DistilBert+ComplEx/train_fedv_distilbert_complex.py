# train_fedv_distilbert_complex.py — Federated QA | DistilBERT + ComplEx | 7 silos | Client7
#
# Differences from Client5 (5 silos):
#   - Imports SILO_F_KB, SILO_G_KB
#   - Loads model_f, model_g
#   - 7 optimizers: optim_a … optim_g
#   - server() forward: h_a … h_g (each (N, 512) ComplEx embeddings)
#   - Gradient clipping on 7 models
#   - L2 normalisation on 7 models (ComplEx requires this, unlike RotatE)
    ...
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            ...
            f.write("  FedV-KGQA  —  Federated QA Training Log"
                    "  [ComplEx + DistilBERT | 7 silos | Client7]\n")
...
def train():
    ...
    for epoch in range(1, QA_EPOCHS + 1):
        ...
        for batch_idx, (questions, topic_ids,
            ...
            # ComplEx: ent_embed.weight ∈ (N, 2d) = (N, 512) [re|im]
            loss, _ = server(
                questions, topic_ids,
                ...
                model_f.ent_embed.weight,    # h_F: (N, 512) [re|im]
                model_g.ent_embed.weight,    # h_G: (N, 512) [re|im]
                answer_ids_batch, candidate_ids,
                device, margin=QA_MARGIN
            )

            # Backward — ∂L/∂h_joint splits into 7 slices of (N, 512)
            loss.backward()
    ...
    print("\n" + "=" * 65)
    print("  FINAL TEST RESULTS (ComplEx + DistilBERT | 2-hop | 7 silos | Client7)")
    print("=" * 65)
