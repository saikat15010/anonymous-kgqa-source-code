# evaluate_roberta_complex.py — Evaluation | RoBERTa + ComplEx | 7 silos | Client7
#
# Differences from Client5 evaluate_roberta_complex.py (5 silos):
#   - evaluate() accepts model_f, model_g in addition to model_a…model_e
#   - server.fuse() called with 7 embeddings
#
# ComplEx: get_entity_embeddings() returns (N, 512) = (N, 2×256) per silo.
# Server fuses into h_joint ∈ ℝ^3584.
# RoBERTa: out.last_hidden_state[:, 0, :] → (B, 768) — <s> token.

import torch
from collections import defaultdict
from config_roberta_complex import USE_TOPIC_ANCHORING
...
def evaluate(server, model_a, model_b, model_c, model_d, model_e,
             model_f, model_g, qa_loader, device, per_type=True):
    """
    ComplEx: get_entity_embeddings() returns (N, 512) = (N, 2×256) per silo.
    Server fuses 7 embeddings into h_joint ∈ ℝ^3584.
    RoBERTa encoder: out.last_hidden_state[:, 0, :] → (B, 768) → MLP → (B, 3584).
    """
    server.eval()
    model_a.eval(); model_b.eval(); model_c.eval()
    model_d.eval(); model_e.eval(); model_f.eval(); model_g.eval()
    ...
    with torch.no_grad():
        ...
        h_joint = server.fuse(h_a, h_b, h_c, h_d, h_e, h_f, h_g)   # (N, 3584)
...
def print_results(split_name, overall, per_type=None):
    ...
    print(f"\n{'─'*70}")
    print(f"  {split_name} Results  [RoBERTa + ComplEx | 7 silos | Client7]")
    print(f"{'─'*70}")
