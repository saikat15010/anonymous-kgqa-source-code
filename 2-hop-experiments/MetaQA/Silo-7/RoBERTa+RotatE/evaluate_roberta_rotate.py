# evaluate_roberta_rotate.py — Evaluation | RoBERTa + RotatE | 7 silos | Client7
#
# Differences from Client5 evaluate_roberta_rotate.py (5 silos):
#   - evaluate() accepts model_f, model_g in addition to model_a…model_e
#   - server.fuse() called with 7 embeddings
#
# RotatE: get_entity_embeddings() returns (N, 512) = (N, 2×256) per silo.
# Server fuses into h_joint ∈ ℝ^3584.
# RoBERTa: out.last_hidden_state[:, 0, :] → (B, 768) — <s> token.

import torch
from collections import defaultdict
from config_roberta_rotate import USE_TOPIC_ANCHORING
...
def evaluate(server, model_a, model_b, model_c, model_d, model_e,
    ...
    """
    RotatE: get_entity_embeddings() returns (N, 512) per silo.
...
def print_results(split_name, overall, per_type=None):
    ...
    print(f"\n{'─'*70}")
    print(f"  {split_name} Results  [RoBERTa + RotatE | 7 silos | Client7]")
    print(f"{'─'*70}")
