# train_kge_distilbert_distmult.py — Train DistMult on 7 silos | DistilBERT | Client7
#
# Differences from Client5 (5 silos):
#   - Imports SILO_F_KB, SILO_G_KB from config_distilbert_distmult
...
#
# DistMult: L2-normalise entity embeddings after each gradient step.
...
def train_silo(silo_name, kb_path, shared_entity2id, device, logger):
    print(f"\n{'='*60}")
    print(f"  Training DistMult — {silo_name}  [Client7 | DistilBERT | 7 silos]")
    print(f"{'='*60}")
