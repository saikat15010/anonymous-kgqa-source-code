# evaluate_roberta_transe.py — Evaluation | RoBERTa + TransE | 7 silos | Client7
#
# Differences from Client5 evaluate_roberta_transe.py (5 silos):
#   - evaluate() accepts model_f, model_g in addition to model_a…model_e
#   - server.fuse() called with 7 embeddings
#
# TransE: get_entity_embeddings() returns (N, 256) per silo.
# Server fuses into h_joint ∈ ℝ^1792.
...
def evaluate(server, model_a, model_b, model_c, model_d, model_e,
    ...
    """
    TransE: get_entity_embeddings() returns (N, 256) per silo.
    Server fuses 7 embeddings into h_joint ∈ ℝ^1792.
    RoBERTa encoder: out.last_hidden_state[:, 0, :] → (B, 768) — <s> token.
    """
    server.eval()
...
def print_results(split_name, overall, per_type=None):
    ...
    print(f"\n{'─'*70}")
    print(f"  {split_name} Results  [RoBERTa + TransE | 7 silos | Client7]")
    print(f"{'─'*70}")
