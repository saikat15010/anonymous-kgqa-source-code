# server_distilbert_rotate.py — Federated server | DistilBERT + RotatE | 7 silos | Client7
#
# Differences from Client5 server_distilbert_rotate.py (5 silos):
...
#
# RotatE entity embeddings are 2d-dim [re|im]:
#   entity_dim = 512 per silo, joint_dim = 3584 for 7 silos.
...
class QuestionEncoder(nn.Module):
    """
    DistilBERT [CLS] → MLP → q_embed ∈ ℝ^3584
...
class FedVServer(nn.Module):
    """
    Federated server for DistilBERT + RotatE | 7 silos | Client7.
    ...
    def forward(self, questions, topic_ids,
        ...
        """
        h_a … h_g : (N, 512) — RotatE entity embeddings [re|im]
        h_joint   : (N, 3584)
        q_embed   : (B, 3584)
        """
