# train_fedv_distilbert_rotate.py — Federated QA | DistilBERT + RotatE | 7 silos | Client7
#
# Differences from Client5 (5 silos):
#   - Imports SILO_F_KB, SILO_G_KB
#   - Loads model_f, model_g
#   - 7 optimizers: optim_a … optim_g
#   - server() forward: h_a … h_g (each (N, 512) RotatE embeddings)
#   - Gradient clipping on 7 models
#   - NO L2 normalisation loop (RotatE does not need it)
    ...
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            ...
            f.write("  FedV-KGQA  —  Federated QA Training Log"
                    "  [RotatE + DistilBERT | 7 silos | Client7]\n")
...
def train():
    ...
    print(f"Joint embedding dim (7×2d) : {JOINT_DIM}")
    ...
    for epoch in range(1, QA_EPOCHS + 1):
        ...
        for batch_idx, (questions, topic_ids,
            ...
            loss, _ = server(
                ...
                model_f.ent_embed.weight,    # h_F: (N, 512)
                model_g.ent_embed.weight,    # h_G: (N, 512)
            ...
            # (unit modulus applies only to relation phase angles,
    ...
    print("\n" + "=" * 65)
    print("  FINAL TEST RESULTS (RotatE + DistilBERT | 2-hop | 7 silos | Client7)")
    print("=" * 65)
