# evaluate_complex.py — Evaluation for BERT + ComplEx | 7 silos | Client7
#
# Differences from Client5 evaluate_complex.py (5 silos):
...
from config_complex import USE_TOPIC_ANCHORING
...
def evaluate(server, model_a, model_b, model_c, model_d, model_e,
    ...
    """
    ComplEx: get_entity_embeddings() returns (N, 512) per silo.
...
def print_results(split_name, overall, per_type=None):
    ...
    print(f"\n{'─'*70}")
    print(f"  {split_name} Results  [BERT + ComplEx | 7 silos | Client7]")
    print(f"{'─'*70}")
