# train_kge_complex.py — Train ComplEx independently on each of the 7 silos
# BERT + ComplEx | 7 silos | Client7
#
# Differences from Client5 train_kge_complex.py (5 silos):
#   - Imports SILO_F_KB, SILO_G_KB, KGE_WEIGHT_DECAY from config_complex
...
class KGELogger:
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            f.write("=" * 70 + "\n")
            f.write("  FedV-KGQA  —  KGE Training Log  [ComplEx | 7 silos | Client7]\n")
...
def train_silo(silo_name, kb_path, shared_entity2id, device, logger):
    print(f"\n{'='*60}")
    print(f"  Training ComplEx — {silo_name}  [Client7 | BERT | 7 silos]")
    ...
    # ComplEx: Adam with L2 weight decay (standard)
