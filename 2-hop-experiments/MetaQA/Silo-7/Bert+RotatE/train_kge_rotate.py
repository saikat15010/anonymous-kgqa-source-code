# train_kge_rotate.py — Train RotatE independently on each of the 7 silos
# BERT + RotatE | 7 silos | Client7
#
# Differences from Client5 train_kge_rotate.py (5 silos):
#   - Imports SILO_F_KB, SILO_G_KB from config_rotate
#   - Builds shared entity index from 7 silos (a through g)
#   - Trains 7 silos sequentially
#
# RotatE: plain Adam (no weight_decay).
# NO post-step L2 normalisation on entity embeddings (unlike TransE/DistMult).
# Unit modulus constraint on relations enforced via cos/sin in scoring.
...
class KGELogger:
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            f.write("=" * 70 + "\n")
            f.write("  FedV-KGQA  —  KGE Training Log  [RotatE | 7 silos | Client7]\n")
    ...
    def start_silo(self, silo_name, num_entities, num_relations, num_triples):
        ...
        with open(self.txt_path, "a") as f:
            ...
            f.write(f"  Entity emb dim : {ENTITY_DIM:>10,}  (2×{KGE_EMBED_DIM})\n")
            f.write("-" * 70 + "\n")
...
def train_silo(silo_name, kb_path, shared_entity2id, device, logger):
    print(f"\n{'='*60}")
    print(f"  Training RotatE — {silo_name}  [Client7 | BERT | 7 silos]")
    print(f"  Entity dim: {ENTITY_DIM} (= 2×{KGE_EMBED_DIM})")
    print(f"{'='*60}")
...
def main():
    ...
    # Build shared entity index from ALL 7 silos
