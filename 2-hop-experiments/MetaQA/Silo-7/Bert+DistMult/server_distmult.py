# server_distmult.py — Federated server for BERT + DistMult | 7 silos | Client7
#
# Differences from Client5 server.py (5 silos):
#   - fuse() takes 7 args (h_a … h_g)
#   - output_dim = 7 * embed_dim = 1792  (not 5×256 = 1280)
#   - forward() signature takes h_a, h_b, h_c, h_d, h_e, h_f, h_g
#   - Imports from config_distmult
#
# DistMult entity dim = 256 per silo (same as TransE) — server architecture
# identical to Bert+TransE Client7 except for the config import.
...
class FedVServer(nn.Module):
    """
    Federated server for BERT + DistMult | 7 silos | Client7.
    ...
    def forward(self, questions, topic_ids,
        ...
        """
        h_a … h_g : (N, 256) — DistMult entity embeddings from 7 silos
        h_joint   : (N, 1792)
        q_embed   : (B, 1792)
        """
