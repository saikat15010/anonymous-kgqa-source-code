# evaluate_roberta_distmult.py — Evaluation | RoBERTa + DistMult | 7 silos | Client7
#
# Differences from Client5 evaluate_roberta_distmult.py (5 silos):
#   - evaluate() accepts model_f, model_g in addition to model_a…model_e
#   - server.fuse() called with 7 embeddings
#
# DistMult: get_entity_embeddings() returns (N, 256) per silo.
...
def print_results(split_name, overall, per_type=None):
    ...
    print(f"\n{'─'*70}")
    print(f"  {split_name} Results  [RoBERTa + DistMult | 7 silos | Client7]")
    print(f"{'─'*70}")
