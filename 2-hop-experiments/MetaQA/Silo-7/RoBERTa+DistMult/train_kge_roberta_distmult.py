# train_kge_roberta_distmult.py — Train DistMult on 7 silos | RoBERTa | Client7
#
# Differences from Client5 (5 silos):
#   - Imports SILO_F_KB, SILO_G_KB from config_roberta_distmult
...
#
# DistMult: scoring φ(h,r,t) = Σ h_i·r_i·t_i; L2-normalise entity embeddings
# after each gradient step (same as TransE).
...
class KGELogger:
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            f.write("=" * 70 + "\n")
            f.write("  FedV-KGQA  —  KGE Training Log  [DistMult | 7 silos | Client7]\n")
...
def train_silo(silo_name, kb_path, shared_entity2id, device, logger):
    print(f"\n{'='*60}")
    print(f"  Training DistMult — {silo_name}  [Client7 | RoBERTa | 7 silos]")
    print(f"{'='*60}")
    ...
    for epoch in range(1, KGE_EPOCHS + 1):
        ...
        for h, r, t_pos in loader:
            ...
            # DistMult: L2-normalise entity embeddings each step
            with torch.no_grad():
