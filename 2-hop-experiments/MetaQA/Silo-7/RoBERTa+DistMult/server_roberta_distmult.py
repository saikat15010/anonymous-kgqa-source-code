# server_roberta_distmult.py — Federated server | RoBERTa + DistMult | 7 silos | Client7
#
# Differences from Client5 server_roberta_distmult.py (5 silos):
...
#
# RoBERTa implementation notes (unchanged):
#   - RobertaTokenizer + RobertaModel (no token_type_ids)
#   - out.last_hidden_state[:, 0, :] = <s> token = sequence representation
#   - 768-dim output → same MLP input dim as BERT/DistilBERT
# DistMult entity dim = 256 per silo (same as TransE).
# DistMult: L2-normalise entity embeddings after each update step.
...
from config_roberta_distmult import (
    ...
    KGE_EMBED_DIM, USE_TOPIC_ANCHORING, ROBERTA_DIM, JOINT_DIM
)


class QuestionEncoder(nn.Module):
    """
    RoBERTa <s> token → MLP → q_embed ∈ ℝ^1792
...
class FedVServer(nn.Module):
    """
    Federated server for RoBERTa + DistMult | 7 silos | Client7.
    ...
    def forward(self, questions, topic_ids,
        ...
        """
        h_a … h_g : (N, 256) — DistMult entity embeddings
        h_joint   : (N, 1792)
        q_embed   : (B, 1792)
        """
