# train_kge_complex.py — Train ComplEx independently on each of the 5 silos
# Client5 | BERT + ComplEx | 5 silos
#
# Differences from Client3 train_kge_complex.py (3 silos):
#   - Imports SILO_D_KB, SILO_E_KB, KGE_WEIGHT_DECAY from config_complex
#   - Builds shared entity index from 5 silo KBs
#   - Trains 5 silos sequentially: silo_a through silo_e
#
# ComplEx specifics (identical to Client3):
#   - Scoring: φ(h,r,t) = Re(<h, r, conj(t)>) — Hermitian dot product
#   - Entity AND relation embeddings are 2d-dim complex [re|im]
#   - Adam with weight_decay=1e-6 (standard L2 regularisation for ComplEx)
...
def train_silo(silo_name, kb_path, shared_entity2id, device, logger):
    print(f"\n{'='*60}")
    print(f"  Training ComplEx — {silo_name}  [Client5 | 5 silos]")
    ...
    # ComplEx: Adam with L2 weight decay
...
def main():
    ...
    # Train ComplEx on each of the 5 silos independently
