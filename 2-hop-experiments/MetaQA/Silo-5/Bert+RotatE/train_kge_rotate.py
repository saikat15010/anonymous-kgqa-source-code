# train_kge_rotate.py — Train RotatE independently on each of the 5 silos
# Client5 | BERT + RotatE | 5 silos
#
# Differences from Client3 train_kge_rotate.py (3 silos):
#   - Imports SILO_D_KB, SILO_E_KB from config_rotate
#   - Builds shared entity index from 5 silo KBs
#   - Trains 5 silos sequentially: silo_a through silo_e
#
# RotatE specifics (identical to Client3):
#   - Entity embeddings: 2d-dim [re|im] stored as 2*KGE_EMBED_DIM
#   - Relation embeddings: d-dim phase angles (unit modulus via cos/sin)
#   - L2-normalise entity embeddings after each gradient step
#   - No weight_decay on optimizer
...
class KGELogger:
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            f.write("=" * 70 + "\n")
            f.write("  FedV-KGQA  —  KGE Training Log  [RotatE | 5 silos | Client5]\n")
...
def train_silo(silo_name, kb_path, shared_entity2id, device, logger):
    print(f"\n{'='*60}")
    print(f"  Training RotatE — {silo_name}  [Client5 | 5 silos]")
    ...
    for epoch in range(1, KGE_EPOCHS + 1):
        ...
        for h, r, t_pos in loader:
            ...
            # RotatE: L2-normalise entity embeddings after each step
            with torch.no_grad():
...
def main():
    ...
    # Train RotatE on each of the 5 silos independently
