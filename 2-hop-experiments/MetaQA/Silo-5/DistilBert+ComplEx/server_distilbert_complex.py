# server_distilbert_complex.py — Federated server | DistilBERT + ComplEx | 5 silos | Client5
#
# Differences from Client3 server_distilbert_complex.py (3 silos):
...
#   entity_dim = 512 per silo, joint_dim = 2560 for 5 silos
# Identical structure to server_distilbert_rotate.py — both use 2d entity embeddings.
# DistilBERT encoder is frozen; only MLP trains.
...
class QuestionEncoder(nn.Module):
    """
    DistilBERT [CLS] → MLP → q_embed ∈ ℝ^2560
    ...
    DistilBERT is frozen; only MLP trains during federated QA.
    """
...
class FedVServer(nn.Module):
    """
    Federated server for DistilBERT + ComplEx | 5 silos | Client5.
    ...
    ComplEx advantage: Hermitian dot product handles asymmetric relations
    (e.g. directed_by ≠ directed) — critical for OWL inverseOf enrichment.
    """
