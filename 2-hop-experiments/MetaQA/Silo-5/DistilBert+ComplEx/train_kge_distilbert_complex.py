# train_kge_distilbert_complex.py — Train ComplEx on 5 silos | DistilBERT | Client5
#
# Differences from Client3 (3 silos):
#   - Imports SILO_D_KB, SILO_E_KB, KGE_WEIGHT_DECAY from config_distilbert_complex
...
#
# ComplEx specifics:
#   - Adam with weight_decay=1e-6 (L2 regularisation, standard for ComplEx)
#   - L2-normalise entity embeddings after each gradient step
#   - Entity embeddings: 2d-dim [re|im]; relation embeddings: 2d-dim [re|im]
...
def train_silo(silo_name, kb_path, shared_entity2id, device, logger):
    print(f"\n{'='*60}")
    print(f"  Training ComplEx — {silo_name}  [Client5 | DistilBERT | 5 silos]")
