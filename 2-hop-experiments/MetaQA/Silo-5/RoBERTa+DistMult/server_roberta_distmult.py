# server_roberta_distmult.py — Federated server | RoBERTa + DistMult | 5 silos | Client5
#
# Differences from Client3 server_roberta_distmult.py (3 silos):
...
#
# RoBERTa implementation notes (identical to Client3):
...
class QuestionEncoder(nn.Module):
    """
    RoBERTa <s> token → MLP → q_embed ∈ ℝ^1280
...
class FedVServer(nn.Module):
    """
    Federated server for RoBERTa + DistMult | 5 silos | Client5.
    ...
    def forward(self, questions, topic_ids, h_a, h_b, h_c, h_d, h_e,
        ...
        """
        h_a … h_e : (N, 256) — DistMult entity embeddings
        h_joint   : (N, 1280)
        q_embed   : (B, 1280)
        """
