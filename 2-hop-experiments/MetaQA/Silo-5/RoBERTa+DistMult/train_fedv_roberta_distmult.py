# train_fedv_roberta_distmult.py — Federated QA | RoBERTa + DistMult | 5 silos | Client5
#
# Differences from Client3 (3 silos):
#   - Imports SILO_D_KB, SILO_E_KB
#   - Loads model_d, model_e
#   - 5 optimizers: optim_a … optim_e
#   - server() forward: h_a … h_e  (each (N, 256) DistMult embeddings)
    ...
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            ...
            f.write("  FedV-KGQA  —  Federated QA Training Log"
                    "  [DistMult + RoBERTa | 5 silos | Client5]\n")
...
def train():
    ...
    print(f"RoBERTa → MLP output         : {5 * KGE_EMBED_DIM}  (matches joint dim)")
    ...
    for epoch in range(1, QA_EPOCHS + 1):
        ...
        for batch_idx, (questions, topic_ids,
            ...
            # DistMult: L2-normalise entity embeddings after each update
            with torch.no_grad():
    ...
    print("\n" + "=" * 65)
    print("  FINAL TEST RESULTS (DistMult + RoBERTa | 2-hop | 5 silos | Client5)")
    print("=" * 65)
