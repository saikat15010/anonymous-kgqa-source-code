# evaluate_roberta_distmult.py — Evaluation | RoBERTa + DistMult | 5 silos | Client5
#
# Differences from Client3 evaluate_roberta_distmult.py:
...
def evaluate(server, model_a, model_b, model_c, model_d, model_e,
    ...
    DistMult: get_entity_embeddings() returns (N, 256) per silo.
    Server fuses into h_joint ∈ ℝ^1280.
    RoBERTa encoder: out.last_hidden_state[:, 0, :] → (B, 768).
    """
    server.eval()
...
def print_results(split_name, overall, per_type=None):
    ...
    print(f"\n{'─'*70}")
    print(f"  {split_name} Results  [RoBERTa + DistMult | 5 silos | Client5]")
    print(f"{'─'*70}")
