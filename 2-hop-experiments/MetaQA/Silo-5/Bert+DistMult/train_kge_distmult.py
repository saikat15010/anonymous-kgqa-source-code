# train_kge_distmult.py — Train DistMult independently on each of the 5 silos
# Client5 | BERT + DistMult | 5 silos
#
# Differences from Client3 train_kge_distmult.py (3 silos):
...
#
# DistMult specifics (identical to Client3):
#   - Scoring: φ(h,r,t) = <h,r,t> = Σ h_i·r_i·t_i
...
class KGELogger:
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            f.write("=" * 70 + "\n")
            f.write("  FedV-KGQA  —  KGE Training Log  [DistMult | 5 silos | Client5]\n")
...
def train_silo(silo_name, kb_path, shared_entity2id, device, logger):
    print(f"\n{'='*60}")
    print(f"  Training DistMult — {silo_name}  [Client5 | 5 silos]")
    print(f"{'='*60}")
...
def main():
    ...
    # Train DistMult on each of the 5 silos independently
