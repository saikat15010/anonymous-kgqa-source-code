# evaluate.py — Evaluation for BERT + DistMult | 5 silos | Client5
#
...
def evaluate(server, model_a, model_b, model_c, model_d, model_e,
    ...
    """
    For DistMult: get_entity_embeddings() returns (N, 256) per silo.
    ...
    with torch.no_grad():
        ...
        h_joint = server.fuse(h_a, h_b, h_c, h_d, h_e)
...
def print_results(split_name, overall, per_type=None):
    ...
    print(f"\n{'─'*70}")
    print(f"  {split_name} Results  [BERT + DistMult | 5 silos | Client5]")
    print(f"{'─'*70}")
