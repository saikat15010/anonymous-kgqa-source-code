# evaluate_roberta_complex.py — Evaluation | RoBERTa + ComplEx | 5 silos | Client5
#
# Differences from Client3 evaluate_roberta_complex.py:
#   - evaluate() accepts model_d, model_e
#   - server.fuse() called with 5 embeddings
#
# ComplEx: get_entity_embeddings() returns (N, 512) = (N, 2×256) per silo.
# Server fuses into h_joint ∈ ℝ^2560.
...
def evaluate(server, model_a, model_b, model_c, model_d, model_e,
    ...
    ComplEx: get_entity_embeddings() returns (N, 512) = (N, 2×256) per silo.
    Server fuses into h_joint ∈ ℝ^2560.
    RoBERTa encoder: out.last_hidden_state[:, 0, :] → (B, 768) → MLP → (B, 2560).
    """
    server.eval()
    ...
    with torch.no_grad():
        ...
        h_joint = server.fuse(h_a, h_b, h_c, h_d, h_e)   # (N, 2560)
...
def print_results(split_name, overall, per_type=None):
    ...
    print(f"\n{'─'*70}")
    print(f"  {split_name} Results  [RoBERTa + ComplEx | 5 silos | Client5]")
    print(f"{'─'*70}")
