# evaluate.py — Evaluation for BERT + TransE | 5 silos | Client5
#
# Differences from Client3 evaluate.py:
#   - evaluate() accepts model_d, model_e in addition to model_a/b/c
#   - server.fuse() called with 5 embeddings
#   - Imports USE_TOPIC_ANCHORING from config (same flag, same value)
#
# MRR, Hits@k logic and per-type detection are identical to Client3.
...
def evaluate(server, model_a, model_b, model_c, model_d, model_e,
    ...
    For TransE: get_entity_embeddings() returns (N, 256) per silo.
    Server fuses into h_joint ∈ ℝ^1280.
    """
    server.eval()
    model_a.eval(); model_b.eval(); model_c.eval()
    model_d.eval(); model_e.eval()
    ...
    with torch.no_grad():
        ...
        h_joint = server.fuse(h_a, h_b, h_c, h_d, h_e)   # (N, 1280)
...
def print_results(split_name, overall, per_type=None):
    ...
    print(f"\n{'─'*70}")
    print(f"  {split_name} Results  [BERT + TransE | 5 silos | Client5]")
    print(f"{'─'*70}")
