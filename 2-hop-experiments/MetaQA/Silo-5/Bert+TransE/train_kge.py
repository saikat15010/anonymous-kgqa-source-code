# train_kge.py — Train TransE independently on each of the 5 silos
# Client5 | BERT + TransE | 5 silos
#
# Differences from Client3 train_kge.py (3 silos):
...
#
# Everything else — TransE scoring, margin loss, L2 normalisation,
# logger, checkpoint format — is identical to Client3.
...
class KGELogger:
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            f.write("=" * 70 + "\n")
            f.write("  FedV-KGQA  —  KGE Training Log  [TransE | 5 silos | Client5]\n")
...
def train_silo(silo_name, kb_path, shared_entity2id, device, logger):
    print(f"\n{'='*60}")
    print(f"  Training TransE — {silo_name}  [Client5 | 5 silos]")
    print(f"{'='*60}")
...
def main():
    ...
    # Train TransE on each of the 5 silos independently
