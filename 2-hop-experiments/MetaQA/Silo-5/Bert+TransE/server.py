# server.py — Federated server for BERT + TransE | 5 silos | Client5
#
...
#
# Everything else — BERT encoder, MLP, cosine scoring, margin ranking loss,
...
class QuestionEncoder(nn.Module):
    ...
    TransE entity embeddings ∈ ℝᵈ = ℝ^256 per silo.
...
class FedVServer(nn.Module):
    """
    Federated server for BERT + TransE | 5 silos | Client5.
    ...
    Gradient splitting at concat boundary:
        ...
        Each silo receives only its own slice.
    """
    ...
    def score_candidates(self, q_embed, h_joint, candidate_ids):
        ...
        h_cands  = h_joint[safe_ids]                                # (B, K, 1280)
        q_norm   = F.normalize(q_embed, p=2, dim=-1).unsqueeze(1)  # (B, 1, 1280)
        h_norm   = F.normalize(h_cands, p=2, dim=-1)               # (B, K, 1280)
    ...
    def forward(self, questions, topic_ids, h_a, h_b, h_c, h_d, h_e,
        ...
        """
        h_a … h_e    : (N, 256) — TransE entity embeddings from 5 silos
        h_joint      : (N, 1280) — after fusion
        q_embed      : (B, 1280) — from BERT + MLP
        """
        h_joint = self.fuse(h_a, h_b, h_c, h_d, h_e)          # (N, 1280)
        q_embed = self.question_encoder(questions, device)      # (B, 1280)
