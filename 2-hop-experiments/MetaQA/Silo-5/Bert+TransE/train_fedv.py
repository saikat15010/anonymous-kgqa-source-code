# train_fedv.py — Federated QA training | BERT + TransE | 5 silos | Client5
#
# Differences from Client3 train_fedv.py (3 silos):
#   - Imports SILO_D_KB, SILO_E_KB from config
...
#
# Everything else — logger, collate, QA datasets, training loop,
# dev/test evaluation cadence — is identical to Client3.
...
def train():
    ...
    print(f"Joint embedding dim (5d)   : {5 * KGE_EMBED_DIM}")
    ...
    # ── Neighbor index + candidates (all 5 KB paths) ───────────────────────────
    ...
    # ── Load 5 pretrained TransE checkpoints ──────────────────────────────────
    ...
    for epoch in range(1, QA_EPOCHS + 1):
        ...
        for batch_idx, (questions, topic_ids,
            ...
            # Forward — pass all 5 silo entity embeddings
            loss, _ = server(
                questions, topic_ids,
            ...
            # Backward — gradient splits into 5 slices of (N, 256)
            loss.backward()
            ...
            # L2-normalise TransE entity embeddings after each update
            with torch.no_grad():
    ...
    print("\n" + "=" * 65)
    print("  FINAL TEST RESULTS (TransE + BERT | 2-hop | 5 silos | Client5)")
    print("=" * 65)
