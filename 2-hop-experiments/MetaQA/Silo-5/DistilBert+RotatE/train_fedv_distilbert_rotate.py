# train_fedv_distilbert_rotate.py — Federated QA | DistilBERT + RotatE | 5 silos | Client5
#
# Differences from Client3 (3 silos):
#   - Imports SILO_D_KB, SILO_E_KB
#   - Loads model_d, model_e
#   - 5 optimizers: optim_a … optim_e
#   - server() forward: h_a … h_e  (each (N, 512) RotatE embeddings)
#   - Gradient clipping on 5 models
#   - NO L2 normalisation (RotatE does not need it)
#   - build_neighbor_index with 5 KB paths
#   - evaluate() with model_a … model_e
#   - _save/_load with model_a … model_e
    ...
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            ...
            f.write("  FedV-KGQA  —  Federated QA Training Log"
                    "  [RotatE + DistilBERT | 5 silos | Client5]\n")
...
def train():
    ...
    print("\nBuilding neighbor index (all 5 silo KBs)...")
    ...
    model_d = load_rotate("silo_d", device)
    model_e = load_rotate("silo_e", device)
    ...
    for epoch in range(1, QA_EPOCHS + 1):
        ...
        for batch_idx, (questions, topic_ids,
            ...
            # RotatE: ent_embed.weight ∈ (N, 2d) = (N, 512)
            loss, _ = server(
                questions, topic_ids,
                model_a.ent_embed.weight,    # h_A: (N, 512)
                model_b.ent_embed.weight,    # h_B: (N, 512)
                model_c.ent_embed.weight,    # h_C: (N, 512)
                model_d.ent_embed.weight,    # h_D: (N, 512)
                model_e.ent_embed.weight,    # h_E: (N, 512)
            ...
            #  which are fixed after Phase 1 KGE training)
        ...
        if epoch % 2 == 0 or epoch == QA_EPOCHS:
            ...
            if dev_results["hits@1"] > best_hits1:
                ...
                best_epoch = epoch
                _save(server, model_a, model_b, model_c, model_d, model_e, "best")
    ...
    print("\n" + "=" * 65)
    print("  FINAL TEST RESULTS (RotatE + DistilBERT | 2-hop | 5 silos | Client5)")
    print("=" * 65)
