# server_distilbert_rotate.py — Federated server | DistilBERT + RotatE | 5 silos | Client5
#
# Differences from Client3 server_distilbert_rotate.py (3 silos):
...
#
# RotatE entity embeddings are 2d-dim (complex [re|im]):
...
class QuestionEncoder(nn.Module):
    ...
    RotatE entity embeddings ∈ ℝ^512 (= 2×256 [re|im]) per silo.
...
class FedVServer(nn.Module):
    """
    Federated server for DistilBERT + RotatE | 5 silos | Client5.
    ...
    def forward(self, questions, topic_ids, h_a, h_b, h_c, h_d, h_e,
        ...
        """
        h_a … h_e : (N, 512) — RotatE entity embeddings [re|im]
        h_joint   : (N, 2560)
        q_embed   : (B, 2560)
        """
