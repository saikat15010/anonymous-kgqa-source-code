# train_kge_distilbert_rotate.py — Train RotatE on 5 silos | DistilBERT | Client5
#
# Differences from Client3 (3 silos):
#   - Imports SILO_D_KB, SILO_E_KB from config_distilbert_rotate
...
#
# RotatE: entity embeddings are 2d-dim [re|im]; no post-step normalisation needed.
...
def train_silo(silo_name, kb_path, shared_entity2id, device, logger):
    print(f"\n{'='*60}")
    print(f"  Training RotatE — {silo_name}  [Client5 | DistilBERT | 5 silos]")
    ...
    for epoch in range(1, KGE_EPOCHS + 1):
        ...
        for h, r, t_pos in loader:
            ...
            loss.backward()
            optimizer.step()
            # RotatE: no post-step normalisation needed
