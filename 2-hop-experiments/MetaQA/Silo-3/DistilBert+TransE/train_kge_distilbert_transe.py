# src/train_kge_distilbert_transe.py — Train TransE independently on each of the 3 silos
# (Identical to train_kge.py; KGE training is independent of the QA encoder)
# All results saved to logs_distilbert_transe/run_<timestamp>/kge_train_log.{txt,json}
...
class KGELogger:
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            f.write("=" * 70 + "\n")
            f.write("  FedV-KGQA  —  KGE Training Log  [TransE + DistilBERT]\n")
...
def train_silo(silo_name, kb_path, shared_entity2id, device, logger):
    print(f"\n{'='*60}")
    print(f"  Training TransE — {silo_name}  [for DistilBERT+TransE]")
    print(f"{'='*60}")
    ...
    for epoch in range(1, KGE_EPOCHS + 1):
        ...
        for h, r, t_pos in loader:
            ...
            # L2-normalise entity embeddings after each update
            with torch.no_grad():
