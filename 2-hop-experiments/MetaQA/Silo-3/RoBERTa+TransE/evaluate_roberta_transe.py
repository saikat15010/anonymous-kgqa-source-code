# src/evaluate_roberta_transe.py — Evaluation for RoBERTa + TransE
#
# Identical logic to evaluate.py (Bert+TransE) and all other evaluate_*.py files.
# Imports from config_roberta_transe for correct paths and flags.
...
from config_roberta_transe import USE_TOPIC_ANCHORING
...
def evaluate(server, model_a, model_b, model_c, qa_loader, device,
    ...
    For TransE: model_{a,b,c}.get_entity_embeddings() returns (N, d) = (N, 256).
    Server fuses them into h_joint ∈ ℝ^(3d) = ℝ^768.
    RoBERTa encoder produces (B, 768) — same dim as BERT/DistilBERT output.

    Args:
        server      : FedVServer (RoBERTa + TransE)
        model_a/b/c : TransE models (one per silo)
    ...
    with torch.no_grad():
        # TransE: get_entity_embeddings() returns (N, d) = (N, 256)
        ...
        for questions, topic_ids, answer_ids_batch, candidate_ids in qa_loader:
            ...
            # Encode: RoBERTa <s> token → MLP → (B, 768)
...
def print_results(split_name, overall, per_type=None):
    ...
    print(f"\n{'─'*70}")
    print(f"  {split_name} Results  [RoBERTa + TransE]")
    print(f"{'─'*70}")
