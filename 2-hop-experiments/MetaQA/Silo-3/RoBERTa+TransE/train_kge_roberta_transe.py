# src/train_kge_roberta_transe.py — Train TransE independently on each of the
# 3 silos for the RoBERTa + TransE experiment.
#
...
#   train_kge.py          (Bert+TransE)
#   train_kge_distilbert_transe.py  (DistilBert+TransE)
# with only two differences:
#   1. Imports from config_roberta_transe (different CHECKPOINT_DIR, log dir)
#   2. Log headers label the RoBERTa encoder for traceability
#
# Results saved to:
#   logs_roberta_transe/run_<timestamp>/kge_train_log.txt
#   logs_roberta_transe/run_<timestamp>/kge_train_log.json
...
class KGELogger:
    """
    Writes TransE KGE training results to:
      logs_roberta_transe/run_<id>/kge_train_log.txt
      logs_roberta_transe/run_<id>/kge_train_log.json
    ...
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            f.write("=" * 70 + "\n")
            f.write("  FedV-KGQA  —  KGE Training Log  [TransE + RoBERTa]\n")
...
def train_silo(silo_name, kb_path, shared_entity2id, device, logger):
    """
    Train TransE on one silo's OWL2-enriched knowledge base.

    TransE scoring: φ(h, r, t) = −||h + r − t||₂
    Loss: L = max(0, γ − φ(h,r,t⁺) + φ(h,r,t⁻_hard))
    Post-step: entity embeddings L2-normalised to unit sphere.
    """
    print(f"\n{'='*60}")
    print(f"  Training TransE — {silo_name}  [for RoBERTa+TransE]")
    print(f"{'='*60}")
    ...
    model     = TransE(num_entities, num_relations, KGE_EMBED_DIM,
    ...
    ckpt_path = os.path.join(CHECKPOINT_DIR, f"{silo_name}_transe.pt")
    torch.save({
...
def main():
    ...
    # Save run_id so train_fedv_roberta_transe.py picks up the same log dir
    os.makedirs(CHECKPOINT_DIR, exist_ok=True)
    ...
    # Train TransE on each silo independently
