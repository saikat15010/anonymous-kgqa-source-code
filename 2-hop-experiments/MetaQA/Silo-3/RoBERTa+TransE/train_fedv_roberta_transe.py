# src/train_fedv_roberta_transe.py — Federated QA training: TransE + RoBERTa
#
# Compared to train_fedv.py (Bert+TransE):
#   - Imports FedVServer from server_roberta_transe  (RoBERTa encoder)
#   - Imports evaluate/print_results from evaluate_roberta_transe
#   - Imports config from config_roberta_transe
#   - Log directory is logs_roberta_transe/
#   - All hyperparameters, training logic, and gradient handling are identical
#
# Compared to train_fedv_distilbert_transe.py (DistilBert+TransE):
#   - RoBERTa (125M, 12 layers) instead of DistilBERT (66M, 6 layers)
#   - Same entity dim (256), same joint dim (768), same MLP output dim (768)
#   - RoBERTa is typically slower per batch than DistilBERT but often more accurate
#
# Results saved to:
#   logs_roberta_transe/run_<id>/fedv_train_log.txt
#   logs_roberta_transe/run_<id>/fedv_train_log.json
...
from server_roberta_transe import FedVServer
from evaluate_roberta_transe import evaluate, print_results
...
def load_transe(silo_name, device):
    """Load a pretrained TransE checkpoint from Phase 1 (KGE training)."""
    ckpt = torch.load(
...
def train():
    ...
    # Reuse run_id created by train_kge_roberta_transe.py
    ...
    # ── Load Phase 1 TransE checkpoints ───────────────────────────────────────
    ...
    # ── Server + optimizers ───────────────────────────────────────────────────
    # server.question_encoder.mlp: trained on server (maps 768 → 768)
    # model_{a,b,c}.ent_embed    : (N, 256) — receive gradient slices of h_joint
    ...
    print(f"\nFederated QA training (TransE + RoBERTa)"
    ...
    for epoch in range(1, QA_EPOCHS + 1):
        ...
        for batch_idx, (questions, topic_ids,
            ...
            # Forward — TransE: ent_embed.weight ∈ (N, d) = (N, 256)
            loss, _ = server(
                questions, topic_ids,
            ...
            # Backward — ∂L/∂h_joint splits into three (N,256) slices
            loss.backward()
            ...
            # Re-normalise entity embeddings to unit sphere after each update
            with torch.no_grad():
    ...
    print("\n" + "=" * 65)
    print("  FINAL TEST RESULTS (TransE + RoBERTa | 2-hop | 3 silos)")
    print("=" * 65)
