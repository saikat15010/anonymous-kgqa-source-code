# src/evaluate_roberta_complex.py — Evaluation for RoBERTa + ComplEx
#
# Identical logic to all other evaluate_*.py files.
# Imports from config_roberta_complex for correct paths and flags.
# For ComplEx: model_{a,b,c}.get_entity_embeddings() returns (N, 2d) = (N, 512).
# Server fuses them into h_joint ∈ ℝ^(6d) = ℝ^1536.
...
def detect_answer_type(question):
    """Classify question by expected answer type — identical across all experiments."""
    q = question.lower()
...
def evaluate(server, model_a, model_b, model_c, qa_loader, device,
    ...
    For ComplEx: get_entity_embeddings() returns (N, 2d) = (N, 512) tensors.
    Server fuses into h_joint ∈ ℝ^(6d) = ℝ^1536.
    RoBERTa encoder produces (B, 1536) after MLP — matching joint dim.
    """
    server.eval()
...
def print_results(split_name, overall, per_type=None):
    ...
    print(f"\n{'─'*70}")
    print(f"  {split_name} Results  [RoBERTa + ComplEx]")
    print(f"{'─'*70}")
