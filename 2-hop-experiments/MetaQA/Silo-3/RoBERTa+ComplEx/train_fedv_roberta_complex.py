# src/train_fedv_roberta_complex.py — Federated QA training: ComplEx + RoBERTa
#
# Compared to train_fedv_complex.py (Bert+ComplEx):
#   - Imports FedVServer from server_roberta_complex  (RoBERTa encoder)
#   - Imports evaluate/print_results from evaluate_roberta_complex
#   - Imports config from config_roberta_complex
#   - Log directory is logs_roberta_complex/
#
# Compared to train_fedv_distilbert_complex.py (DistilBert+ComplEx):
#   - RoBERTa (125M, 12 layers) instead of DistilBERT (66M, 6 layers)
#   - Same ENTITY_DIM=512, JOINT_DIM=1536 — architecture is otherwise identical
#   - RoBERTa is slightly slower per batch but typically more accurate
#
# Compared to train_fedv_roberta_distmult.py (RoBERTa+DistMult):
#   - Loads ComplEx checkpoints (not DistMult)
#   - ENTITY_DIM=512 (not 256), JOINT_DIM=1536 (not 768)
#   - ComplEx entity embeddings L2-normalised after each step (same as DistMult)
#
# Results saved to:
#   logs_roberta_complex/run_<id>/fedv_train_log.txt
#   logs_roberta_complex/run_<id>/fedv_train_log.json
...
from server_roberta_complex import FedVServer
from evaluate_roberta_complex import evaluate, print_results
...
def train():
    ...
    # Reuse run_id created by train_kge_roberta_complex.py
    ...
    print(f"\nFederated QA training (ComplEx + RoBERTa)"
    ...
    for epoch in range(1, QA_EPOCHS + 1):
        ...
        for batch_idx, (questions, topic_ids,
            ...
            # Forward — ComplEx: ent_embed.weight ∈ (N, 2d) = (N, 512) [re|im]
            loss, _ = server(
                questions, topic_ids,
    ...
    print("\n" + "=" * 65)
    print("  FINAL TEST RESULTS (ComplEx + RoBERTa | 2-hop | 3 silos)")
    print("=" * 65)
