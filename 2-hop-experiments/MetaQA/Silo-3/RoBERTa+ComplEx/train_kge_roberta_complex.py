# src/train_kge_roberta_complex.py — Train ComplEx independently on each of
# the 3 silos for the RoBERTa + ComplEx experiment.
#
# KGE training is fully independent of the QA encoder — RoBERTa is not
# involved here at all. Logically identical to:
#   train_kge_complex.py              (Bert+ComplEx)
#   train_kge_distilbert_complex.py   (DistilBert+ComplEx)
# with only two differences:
#   1. Imports from config_roberta_complex (different CHECKPOINT_DIR, log dir)
#   2. Log headers label the RoBERTa encoder for traceability
#
# ComplEx specifics:
#   - Scoring: φ(h,r,t) = Re(<h,r,conj(t)>) — Hermitian dot product
#   - Entity AND relation embeddings are 2d-dim complex [re|im]
#   - L2-normalise entity embeddings after each gradient step
#   - Adam optimizer with weight_decay=1e-6 (standard L2 reg for ComplEx)
#   - No unit-modulus constraint on relations (unlike RotatE)
#
# Results saved to:
#   logs_roberta_complex/run_<timestamp>/kge_train_log.txt
#   logs_roberta_complex/run_<timestamp>/kge_train_log.json
...
class KGELogger:
    ...
    Writes ComplEx KGE training results to:
      logs_roberta_complex/run_<id>/kge_train_log.txt
      logs_roberta_complex/run_<id>/kge_train_log.json
    ...
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            f.write("=" * 70 + "\n")
            f.write("  FedV-KGQA  —  KGE Training Log  [ComplEx + RoBERTa]\n")
...
def train_silo(silo_name, kb_path, shared_entity2id, device, logger):
    """
    Train ComplEx on one silo's OWL2-enriched knowledge base.
    ...
    Optimizer: Adam with weight_decay=1e-6 (L2 regularisation — ComplEx standard)
    Post-step: entity embeddings L2-normalised to unit sphere.
    Relation embeddings: unconstrained (unlike RotatE unit modulus).
    """
    print(f"\n{'='*60}")
    print(f"  Training ComplEx — {silo_name}  [for RoBERTa+ComplEx]")
    print(f"  Entity embed dim: {ENTITY_DIM} (= 2 × {KGE_EMBED_DIM} [re|im])")
    print(f"{'='*60}")
...
def main():
    ...
    # Save run_id so train_fedv_roberta_complex.py picks up the same log dir
    os.makedirs(CHECKPOINT_DIR, exist_ok=True)
