# src/evaluate_distilbert_complex.py — Evaluation for DistilBERT + ComplEx
#
# Identical logic to evaluate_complex.py (Bert+ComplEx) but imports from
# config_distilbert_complex so paths and flags are correct for this experiment.
# Answer-type detection, MetricAccumulator, and print format are unchanged
# across all experiments for consistent comparison.
#
# For ComplEx: model_{a,b,c}.get_entity_embeddings() returns (N, 2d) = (N, 512)
# tensors. The server fuses them into h_joint ∈ ℝ^(6d) = ℝ^1536.
# This is identical to the DistilBert+RotatE evaluation — only the loaded
# model type differs.

import torch
from collections import defaultdict
from config_distilbert_complex import USE_TOPIC_ANCHORING
...
def detect_answer_type(question):
    ...
    Identical across all 8 experiments for fair per-type comparison.
    """
    q = question.lower()
...
def evaluate(server, model_a, model_b, model_c, qa_loader, device,
    ...
    For ComplEx: model_{a,b,c}.get_entity_embeddings() returns (N, 2d)
    tensors (real + imaginary parts concatenated). The server fuses them
    into h_joint ∈ ℝ^(6d) = ℝ^1536. Evaluation logic is otherwise identical
    to all other experiments.

    Args:
        server      : FedVServer (DistilBERT + ComplEx)
        model_a/b/c : ComplEx models (one per silo)
    ...
    with torch.no_grad():
        # ComplEx: get_entity_embeddings() returns (N, 2d) = (N, 512)
...
def print_results(split_name, overall, per_type=None):
    """Print evaluation results — format identical across all 8 experiments."""
    ...
    print(f"\n{'─'*70}")
    print(f"  {split_name} Results  [DistilBERT + ComplEx]")
    print(f"{'─'*70}")
