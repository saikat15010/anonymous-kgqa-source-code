# src/train_fedv_distilbert_complex.py — Federated QA training: ComplEx + DistilBERT
#
# Compared to train_fedv_complex.py (Bert+ComplEx):
#   - Imports FedVServer from server_distilbert_complex  (DistilBERT encoder)
#   - Imports evaluate/print_results from evaluate_distilbert_complex
#   - Imports config from config_distilbert_complex
#   - Log directory is logs_distilbert_complex/ (not logs_complex/)
#   - All hyperparameters, training logic, and gradient handling are identical
#
# Compared to train_fedv_distilbert_rotate.py (DistilBert+RotatE):
#   - Loads ComplEx checkpoints (not RotatE)
#   - ComplEx L2-normalises entity embeddings after each step (RotatE does not)
#   - Same ENTITY_DIM=512, JOINT_DIM=1536 — ComplEx and RotatE share the same
#     joint embedding space dimension (both 6d)
#
# Results saved to:
#   logs_distilbert_complex/run_<id>/fedv_train_log.txt
#   logs_distilbert_complex/run_<id>/fedv_train_log.json
...
from server_distilbert_complex import FedVServer
from evaluate_distilbert_complex import evaluate, print_results
...
def load_complex(silo_name, device):
    """Load a pretrained ComplEx checkpoint from Phase 1 (KGE training)."""
    ckpt = torch.load(
        os.path.join(CHECKPOINT_DIR, f"{silo_name}_complex.pt"),
        map_location=device
    )
    model = ComplEx(
...
def train():
    ...
    # Reuse run_id created by train_kge_distilbert_complex.py
    ...
    log_dir = os.path.join(os.path.dirname(CHECKPOINT_DIR),
                           "logs_distilbert_complex", run_id)
    ...
    # ── Load Phase 1 ComplEx checkpoints ──────────────────────────────────────
    print("\nLoading pretrained ComplEx models (Phase 1)...")
    model_a = load_complex("silo_a", device)
    model_b = load_complex("silo_b", device)
    model_c = load_complex("silo_c", device)
    ...
    print(f"\nFederated QA training (ComplEx + DistilBERT)"
    ...
    for epoch in range(1, QA_EPOCHS + 1):
        ...
        for batch_idx, (questions, topic_ids,
            ...
            # Forward — ComplEx: ent_embed.weight ∈ (N, 2d) = (N, 512)
            loss, _ = server(
                questions, topic_ids,
                model_a.ent_embed.weight,    # h_A: (N, 512) [re|im]
                model_b.ent_embed.weight,    # h_B: (N, 512) [re|im]
                model_c.ent_embed.weight,    # h_C: (N, 512) [re|im]
            ...
            # ComplEx: L2-normalise entity embeddings after each update
            # (unlike RotatE which skips this — ComplEx standard practice)
            with torch.no_grad():
    ...
    print("\n" + "=" * 65)
    print("  FINAL TEST RESULTS (ComplEx + DistilBERT | 2-hop | 3 silos)")
    print("=" * 65)
