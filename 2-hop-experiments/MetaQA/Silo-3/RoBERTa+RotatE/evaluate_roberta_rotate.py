# src/evaluate_roberta_rotate.py — Evaluation for RoBERTa + RotatE
#
# Identical logic to all other evaluate_*.py files.
# Imports from config_roberta_rotate for correct paths and flags.
# For RotatE: model_{a,b,c}.get_entity_embeddings() returns (N, 2d) = (N, 512).
...
def evaluate(server, model_a, model_b, model_c, qa_loader, device,
    ...
    For RotatE: get_entity_embeddings() returns (N, 2d) = (N, 512) tensors.
...
def print_results(split_name, overall, per_type=None):
    ...
    print(f"\n{'─'*70}")
    print(f"  {split_name} Results  [RoBERTa + RotatE]")
    print(f"{'─'*70}")
