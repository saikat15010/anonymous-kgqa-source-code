# src/train_fedv_roberta_rotate.py — Federated QA training: RotatE + RoBERTa
#
# Compared to train_fedv_rotate.py (Bert+RotatE):
#   - Imports FedVServer from server_roberta_rotate  (RoBERTa encoder)
#   - Imports evaluate/print_results from evaluate_roberta_rotate
#   - Imports config from config_roberta_rotate
#   - Log directory is logs_roberta_rotate/
#
# Compared to train_fedv_roberta_complex.py (RoBERTa+ComplEx):
#   - Loads RotatE checkpoints  (not ComplEx)
#   - Same ENTITY_DIM=512, JOINT_DIM=1536 — server architecture identical
#   - NO post-step L2 normalisation of entity embeddings  (ComplEx does this)
#
# Compared to train_fedv_distilbert_rotate.py (DistilBert+RotatE):
#   - RoBERTa (125M, 12 layers) instead of DistilBERT (66M, 6 layers)
#   - Same entity/joint dims, same training logic
#
# Results saved to:
#   logs_roberta_rotate/run_<id>/fedv_train_log.txt
#   logs_roberta_rotate/run_<id>/fedv_train_log.json
...
from server_roberta_rotate import FedVServer
from evaluate_roberta_rotate import evaluate, print_results
...
def train():
    ...
    print(f"RoBERTa → MLP output       : {JOINT_DIM}  (matches joint dim)")

    # Reuse run_id created by train_kge_roberta_rotate.py
    ...
    log_dir = os.path.join(os.path.dirname(CHECKPOINT_DIR),
                           "logs_roberta_rotate", run_id)

    config_dict = {
        "kge_model"          : "RotatE",
        "qa_encoder"         : "RoBERTa (roberta-base)",
    ...
    # ── Server + optimizers ───────────────────────────────────────────────────
    # server.question_encoder.mlp: maps 768 → 512 → 1536 (trained on server)
    # model_{a,b,c}.ent_embed    : (N, 512) [re|im] — receive gradient slices
    ...
    print(f"\nFederated QA training (RotatE + RoBERTa)"
    ...
    for epoch in range(1, QA_EPOCHS + 1):
        ...
        for batch_idx, (questions, topic_ids,
            ...
            # Forward — RotatE: ent_embed.weight ∈ (N, 2d) = (N, 512) [re|im]
            loss, _ = server(
                questions, topic_ids,
            ...
            # Backward — ∂L/∂h_joint splits into three (N,512) gradient slices
            loss.backward()
            ...
            # RotatE: NO post-step L2 normalisation on entity embeddings
            # (unit modulus constraint applies only to relation phase angles,
            #  which are fixed after Phase 1 KGE training and stay in silos)
    ...
    print("\n" + "=" * 65)
    print("  FINAL TEST RESULTS (RotatE + RoBERTa | 2-hop | 3 silos)")
    print("=" * 65)
