# src/server_roberta_rotate.py — Federated server for RoBERTa + RotatE
#
# Differences from server_roberta_complex.py (RoBERTa+ComplEx):
#   - Imports from config_roberta_rotate
#   - RotatE and ComplEx share the same entity embedding dimensionality
#     (both 2d=512 per silo, 6d=1536 joint) — server architecture is identical
#
# Differences from server_distilbert_rotate.py (DistilBert+RotatE):
...
class QuestionEncoder(nn.Module):
    ...
    RotatE stores entity embeddings as 2d-dim real vectors [re|im]:
    ...
    (Identical MLP architecture to RoBERTa+ComplEx — both have 6d joint dim)
    ...
    RoBERTa-base properties:
      ...
      - last_hidden_state[:, 0, :] = <s> token (functionally ≡ BERT [CLS])
...
class FedVServer(nn.Module):
    """
    Federated server for RoBERTa + RotatE experiment (3 silos, 2-hop).

    RotatE entity embeddings are 2d-dimensional (complex [re|im] representation):
        h(e) ∈ ℝ^(2d) = ℝ^512  per silo

    Joint embedding after federated fusion:
        h_joint(e) = [h_A(e) || h_B(e) || h_C(e)] ∈ ℝ^(6d) = ℝ^1536

    This matches Bert+RotatE, DistilBert+RotatE, Bert+ComplEx,
    DistilBert+ComplEx, and RoBERTa+ComplEx in joint embedding dimension.

    RotatE scoring (for reference — happens in silos during Phase 1):
        h, t ∈ ℂᵈ,  r_i = e^(iθ_i)  (unit modulus)
        φ(h, r, t) = −||h ∘ r − t||  (complex rotation distance)

    Key difference from ComplEx:
        Relations are constrained to unit modulus (θ only) vs unconstrained ℂᵈ.
        No L2 normalisation of entity embeddings during QA training.
        No weight_decay in KGE Adam optimizer.
    ...
    def fuse(self, h_a, h_b, h_c):
        """
        Concatenate per-silo RotatE entity embeddings.
    ...
    def forward(self, questions, topic_ids, h_a, h_b, h_c,
        ...
        h_a, h_b, h_c : (N, 2d) = (N, 512) — RotatE entity embeddings [re|im]
