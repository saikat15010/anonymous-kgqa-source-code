# config_roberta_rotate.py — Hyperparameters for FedV-KGQA (RotatE + RoBERTa)
# All parameters identical to all other experiments for fair comparison.
# Changes vs RoBERTa+ComplEx   : RotatE scoring (rotation vs Hermitian dot product)
#                                no KGE_WEIGHT_DECAY, no entity L2-norm post-step
# Changes vs Bert+RotatE       : CHECKPOINT_DIR, BERT_MODEL → roberta-base
# Changes vs DistilBert+RotatE : BERT_MODEL → roberta-base (larger, stronger)
# Absolute paths used to avoid BASE_DIR resolution issues.

# ── Paths ──────────────────────────────────────────────────────────────────────
DATA_DIR       = "/home/islamm9/ISWC/Dataset/Client3/data"
CHECKPOINT_DIR = "/home/islamm9/ISWC/Client3/RoBERTa+RotatE/models"
...
# Relations r ∈ ℂᵈ    → stored as d phase angles θ, unit modulus: r_i = e^(iθ_i)
...
#
# RotatE differences from ComplEx:
#   - Unit modulus constraint on relations (cos/sin in scoring)
#   - NO post-step L2 normalisation of entity embeddings
#   - NO weight_decay on KGE Adam optimizer
