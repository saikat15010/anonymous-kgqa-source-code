# src/train_kge_roberta_rotate.py — Train RotatE independently on each of the
# 3 silos for the RoBERTa + RotatE experiment.
#
# KGE training is fully independent of the QA encoder — RoBERTa is not
# involved here at all. Logically identical to:
#   train_kge_rotate.py              (Bert+RotatE)
#   train_kge_distilbert_rotate.py   (DistilBert+RotatE)
# with only two differences:
#   1. Imports from config_roberta_rotate (different CHECKPOINT_DIR, log dir)
#   2. Log headers label the RoBERTa encoder for traceability
#
# RotatE specifics vs ComplEx (same entity dim, different training):
#   - Unit modulus constraint on relations via cos/sin (not unconstrained ℂᵈ)
#   - NO weight_decay on Adam optimizer  (ComplEx uses 1e-6)
#   - NO post-step L2 normalisation of entity embeddings  (ComplEx does this)
#   - Relation embeddings are d-dim phase angles (not 2d-dim complex)
#
# Results saved to:
#   logs_roberta_rotate/run_<timestamp>/kge_train_log.txt
#   logs_roberta_rotate/run_<timestamp>/kge_train_log.json
...
from config_roberta_rotate import (
    SILO_A_KB, SILO_B_KB, SILO_C_KB, CHECKPOINT_DIR,
...
def sample_negatives(batch_size, num_entities, num_neg, device):
    """Uniform random negative tail sampling."""
...
class KGELogger:
    """
    Writes RotatE KGE training results to:
      logs_roberta_rotate/run_<id>/kge_train_log.txt
      logs_roberta_rotate/run_<id>/kge_train_log.json
    ...
    JSON structure:
    {
      ...
      "silos": {
        "silo_a": {
          ...
          "num_relations"    : R,
          "num_triples"      : T,
          "entity_embed_dim" : 512,   ← 2*KGE_EMBED_DIM [re|im]
          "epochs"           : [ {"epoch":e, "avg_loss":l, "elapsed_sec":s}, ... ],
          "final_loss"       : float,
    ...
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            f.write("=" * 70 + "\n")
            f.write("  FedV-KGQA  —  KGE Training Log  [RotatE + RoBERTa]\n")
    ...
    def start_silo(self, silo_name, num_entities, num_relations, num_triples):
        ...
        with open(self.txt_path, "a") as f:
            ...
            f.write(f"  Entity emb dim  : {ENTITY_DIM:>10,}  "
                    f"(2 × {KGE_EMBED_DIM} complex)\n")
            f.write("-" * 70 + "\n")
...
def train_silo(silo_name, kb_path, shared_entity2id, device, logger):
    """
    Train RotatE on one silo's OWL2-enriched knowledge base.

    RotatE scoring:
        h, t ∈ ℂᵈ stored as 2d reals [re|im]
        r_i = e^(iθ_i) — unit modulus, rotation in complex plane
        φ(h, r, t) = −||h ∘ r − t||

    Loss: L = max(0, γ − φ(h,r,t⁺) + φ(h,r,t⁻_hard))

    No post-step normalisation required (unit modulus on relations
    is enforced via cos/sin in the scoring function itself).
    No weight_decay on the optimizer.
    """
    print(f"\n{'='*60}")
    print(f"  Training RotatE — {silo_name}  [for RoBERTa+RotatE]")
    print(f"  Entity embed dim: {ENTITY_DIM} (= 2 × {KGE_EMBED_DIM} complex)")
    print(f"{'='*60}")
    ...
    model     = RotatE(num_entities, num_relations, KGE_EMBED_DIM,
                       norm=KGE_NORM).to(device)
    # RotatE: plain Adam, no weight_decay (unlike ComplEx which uses 1e-6)
    ...
    for epoch in range(1, KGE_EPOCHS + 1):
        ...
        for h, r, t_pos in loader:
            ...
            # (unit modulus is maintained on relation phase angles via cos/sin)
    ...
    ckpt_path = os.path.join(CHECKPOINT_DIR, f"{silo_name}_rotate.pt")
    torch.save({
        "model_state_dict": model.state_dict(),
        "entity2id"       : shared_entity2id,
        "relation2id"     : relation2id,
        "embed_dim"       : KGE_EMBED_DIM,    # complex dim d
        "entity_dim"      : ENTITY_DIM,       # real storage = 2d
        "num_entities"    : num_entities,
        "num_relations"   : num_relations,
    }, ckpt_path)
...
def main():
    ...
    # Save run_id so train_fedv_roberta_rotate.py picks up the same log dir
    os.makedirs(CHECKPOINT_DIR, exist_ok=True)
    ...
    # Build unified shared entity vocabulary across all three silos
    _, e2id_a, _ = build_index(SILO_A_KB)
    _, e2id_b, _ = build_index(SILO_B_KB)
    _, e2id_c, _ = build_index(SILO_C_KB)
    shared_entity2id = build_shared_entity_index(e2id_a, e2id_b, e2id_c)
    ...
    # Train RotatE on each silo independently
