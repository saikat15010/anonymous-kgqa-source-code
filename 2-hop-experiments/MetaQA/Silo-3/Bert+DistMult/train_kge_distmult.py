# src/train_kge_distmult.py — Train DistMult independently on each of the 3 silos
# All results saved to logs_distmult/run_<timestamp>/kge_train_log.{txt,json}
...
class KGELogger:
    ...
    Writes DistMult KGE training results to:
      logs_distmult/run_<id>/kge_train_log.txt
      logs_distmult/run_<id>/kge_train_log.json
    ...
    JSON structure:
    {
      ...
      "silos": {
        "silo_a": {
          ...
          "epochs": [ {epoch, avg_loss, elapsed_sec}, ... ],
    ...
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            f.write("=" * 70 + "\n")
            f.write("  FedV-KGQA  —  KGE Training Log  [DistMult]\n")
...
def train_silo(silo_name, kb_path, shared_entity2id, device, logger):
    print(f"\n{'='*60}")
    print(f"  Training DistMult — {silo_name}")
    print(f"{'='*60}")
    ...
    for epoch in range(1, KGE_EPOCHS + 1):
        ...
        for h, r, t_pos in loader:
            ...
            # DistMult: L2-normalize entity embeddings each step
            with torch.no_grad():
        ...
        # Log every epoch to file; print every 10 to console
