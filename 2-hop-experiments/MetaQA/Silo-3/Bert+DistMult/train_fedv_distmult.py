# src/train_fedv_distmult.py — Federated QA training with DistMult + BERT
# All results saved to logs_distmult/run_<timestamp>/fedv_train_log.{txt,json}
...
class FedVLogger:
    ...
    Writes federated QA training results to:
      logs_distmult/run_<id>/fedv_train_log.txt
      logs_distmult/run_<id>/fedv_train_log.json
    ...
    JSON structure:
    {
      ...
      "qa_encoder": "BERT",
      "config": { ... },
      "train_loss":  [ {epoch, avg_loss, elapsed_sec}, ... ],
      "dev_metrics": [ {epoch, train_avg_loss, overall:{...}, per_type:{...}}, ... ],
      "test_metrics": { best_epoch, overall:{...}, per_type:{...} }
    }
    """
    ...
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            f.write("=" * 70 + "\n")
            f.write("  FedV-KGQA  —  Federated QA Training Log  [DistMult + BERT]\n")
...
def train():
    ...
    # Resolve log directory — reuse run_id from train_kge_distmult.py if present
    ...
    # Server + optimizers — identical to TransE experiment
    ...
    print(f"\nFederated QA training (DistMult + BERT) — {QA_EPOCHS} epochs | "
    ...
    print("\n" + "=" * 60)
    print("  FINAL TEST RESULTS (DistMult + BERT | 2-hop | 3 silos)")
    print("=" * 60)
