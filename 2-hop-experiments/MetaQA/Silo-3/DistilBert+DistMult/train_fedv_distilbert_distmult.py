# src/train_fedv_distilbert_distmult.py — Federated QA training: DistMult + DistilBERT
#
# Compared to train_fedv_distmult.py (Bert+DistMult):
#   - Imports FedVServer from server_distilbert_distmult  (DistilBERT encoder)
#   - Imports evaluate/print_results from evaluate_distilbert_distmult
#   - Imports config from config_distilbert_distmult
#   - Log directory is logs_distilbert_distmult/  (not logs_distmult/)
#   - All hyperparameters, training logic, and gradient handling are identical
#
# Results saved to:
#   logs_distilbert_distmult/run_<id>/fedv_train_log.txt
#   logs_distilbert_distmult/run_<id>/fedv_train_log.json
...
from server_distilbert_distmult import FedVServer
from evaluate_distilbert_distmult import evaluate, print_results
...
def make_collate_fn(candidate_map):
    """
    Collate QA samples into padded batches.
    candidate_ids is padded with -1 where candidates are fewer than max_k.
    """
...
def train():
    ...
    # Reuse run_id created by train_kge_distilbert_distmult.py
    ...
    log_dir = os.path.join(os.path.dirname(CHECKPOINT_DIR),
                           "logs_distilbert_distmult", run_id)
    ...
    # ── Server + optimizers ───────────────────────────────────────────────────
    # Only server.question_encoder.mlp is trained on the server side.
    # model_{a,b,c}.ent_embed receive gradients split at the concat boundary.
    ...
    print(f"\nFederated QA training (DistMult + DistilBERT)"
    ...
    for epoch in range(1, QA_EPOCHS + 1):
        ...
        for batch_idx, (questions, topic_ids,
            ...
            # Forward pass — fuse embeddings, encode question, compute loss
            loss, _ = server(
                questions, topic_ids,
            ...
            # Backward — ∂L/∂h_joint splits at concat boundary automatically
            loss.backward()
    ...
    print("\n" + "=" * 65)
    print("  FINAL TEST RESULTS (DistMult + DistilBERT | 2-hop | 3 silos)")
    print("=" * 65)
