# src/train_kge_distilbert_distmult.py — Train DistMult independently on each
# of the 3 silos for the DistilBERT + DistMult experiment.
#
# KGE training is fully independent of the QA encoder — DistilBERT is not
# involved here at all. This script is identical in logic to train_kge_distmult.py
# (Bert+DistMult) with only two differences:
#   1. Imports from config_distilbert_distmult (different CHECKPOINT_DIR and log dir)
#   2. Log header labels reflect the DistilBERT encoder for traceability
#
# Results saved to:
#   logs_distilbert_distmult/run_<timestamp>/kge_train_log.txt   (human-readable)
#   logs_distilbert_distmult/run_<timestamp>/kge_train_log.json  (machine-readable)
...
class KGELogger:
    ...
    Writes DistMult KGE training results to:
      logs_distilbert_distmult/run_<id>/kge_train_log.txt
      logs_distilbert_distmult/run_<id>/kge_train_log.json
    ...
    JSON structure:
    {
      ...
      "silos": {
        "silo_a": {
          "num_entities": N, "num_relations": R, "num_triples": T,
          "epochs": [ {"epoch": e, "avg_loss": l, "elapsed_sec": s}, ... ],
          "final_loss": float, "training_time_sec": float
        }, ...
      }
    }
    """
    ...
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            f.write("=" * 70 + "\n")
            f.write("  FedV-KGQA  —  KGE Training Log  [DistMult + DistilBERT]\n")
...
def train_silo(silo_name, kb_path, shared_entity2id, device, logger):
    """
    Train DistMult on one silo's enriched knowledge base.
    ...
    """
    print(f"\n{'='*60}")
    print(f"  Training DistMult — {silo_name}  [for DistilBERT+DistMult]")
    print(f"{'='*60}")
...
def main():
    ...
    # Save run_id so train_fedv_distilbert_distmult.py picks up the same log dir
    os.makedirs(CHECKPOINT_DIR, exist_ok=True)
