# src/server_distilbert_rotate.py — Federated server for DistilBERT + RotatE
#
# Differences from server_rotate.py (Bert+RotatE):
#   - DistilBertTokenizer + DistilBertModel instead of BertTokenizer + BertModel
#   - Imports from config_distilbert_rotate
#
...
# topic anchoring, gradient splitting — is identical to all other servers.
...
class QuestionEncoder(nn.Module):
    ...
    RotatE stores entity embeddings as 2d-dim real vectors (real + imaginary
    parts of the complex embedding). Therefore the joint embedding is:

        h_joint = [h_A || h_B || h_C] ∈ ℝ^(3 × 2d) = ℝ^1536

    The MLP output must match this: 768 → 512 → 1536.
    ...
    def forward(self, questions, device):
        ...
        with torch.no_grad():
            # DistilBERT: last_hidden_state[:,0,:] = [CLS] token representation
...
class FedVServer(nn.Module):
    """
    Federated server for DistilBERT + RotatE experiment (3 silos, 2-hop).

    RotatE entity embeddings are 2d-dimensional (complex representation):
        h(e) ∈ ℝ^(2d) = ℝ^512  per silo

    Joint embedding after federated fusion:
        h_joint(e) = [h_A(e) || h_B(e) || h_C(e)] ∈ ℝ^(6d) = ℝ^1536

    This is 2× larger than TransE/DistMult experiments (which use ℝ^768),
    making this and the ComplEx experiment the most expressive embeddings.
    The DistilBERT MLP output is correspondingly 1536-dim.

    Gradient splitting at the concat boundary works the same way:
        ∂L/∂h_joint splits into [∂L/∂h_A || ∂L/∂h_B || ∂L/∂h_C]
        each of size (N, 512) — sent to the respective silo.
    """
    ...
    def forward(self, questions, topic_ids, h_a, h_b, h_c,
        ...
        h_a, h_b, h_c : (N, 2d) = (N, 512)  — RotatE entity embeddings
