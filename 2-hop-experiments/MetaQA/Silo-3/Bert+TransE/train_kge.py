# src/train_kge.py — Train KGE independently on each of the 3 silos
# All results saved to logs/run_<timestamp>/kge_train_log.{txt,json}
...
class KGELogger:
    """
    Writes KGE training results to:
      logs/run_<id>/kge_train_log.txt   — human-readable, one row per epoch
      logs/run_<id>/kge_train_log.json  — machine-readable full history

    JSON structure:
    {
      "run_id": "...",
      "config": { kge_embed_dim, kge_lr, kge_epochs, ... },
      "silos": {
        "silo_a": {
...
def train_silo(silo_name, kb_path, shared_entity2id, device, logger):
    print(f"\n{'='*60}")
    print(f"  Training KGE — {silo_name}")
    print(f"{'='*60}")
    ...
    for epoch in range(1, KGE_EPOCHS + 1):
        ...
        # Log every epoch to file; print every 10 epochs to console
...
def main():
    ...
    # ── Shared log directory (shared with train_fedv.py via run_id file) ──────
    run_id  = datetime.now().strftime("run_%Y%m%d_%H%M%S")
    log_dir = os.path.join(os.path.dirname(CHECKPOINT_DIR), "logs", run_id)
    os.makedirs(log_dir, exist_ok=True)

    # Save run_id so train_fedv.py can write to the same log folder
