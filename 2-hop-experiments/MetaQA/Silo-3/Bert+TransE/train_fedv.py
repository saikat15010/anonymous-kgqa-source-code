# src/train_fedv.py — Federated QA training
# All results saved to logs/run_<timestamp>/fedv_train_log.{txt,json}
# Run ID is shared with train_kge.py so both logs land in the same folder.
...
class FedVLogger:
    ...
    Writes federated QA training results to:
      logs/run_<id>/fedv_train_log.txt   — human-readable
      logs/run_<id>/fedv_train_log.json  — machine-readable
    ...
    JSON structure:
    {
      ...
      "config": { ... },
      "train_loss": [
          {"epoch": 1, "avg_loss": 0.843, "elapsed_sec": 142.3}, ...
      ],
      "dev_metrics": [
          {
            "epoch": 2, "train_avg_loss": 0.712,
            "overall":  {"mrr":0.32,"hits@1":0.28,"hits@3":0.38,"hits@5":0.43,"hits@10":0.51,"total":14872},
            "per_type": {"person":{"mrr":...}, "movie":{...}, ...}
          }, ...
      ],
      "test_metrics": {
          "best_epoch": 18,
    ...
    def log_dev(self, epoch, train_avg_loss, overall, per_type):
        entry = {
            ...
            "per_type"       : {
                t: {k: round(v, 6) for k, v in m.items()}
...
def train():
    ...
    # Reuse the run_id written by train_kge.py if available,
    # so both KGE and QA logs land in the same folder.
    ...
    print("\n" + "=" * 60)
    print("  FINAL TEST RESULTS (2-hop | 3 silos | enriched KB)")
    print("=" * 60)
