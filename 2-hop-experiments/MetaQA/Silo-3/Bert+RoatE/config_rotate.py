# config_rotate.py — Hyperparameters for FedV-KGQA (RotatE + BERT)
...
CHECKPOINT_DIR = "/home/islamm9/ISWC/Client3/Bert+RotatE/models"
...
# ── RotatE KGE ─────────────────────────────────────────────────────────────────
# All values identical to TransE/DistMult config for fair comparison.
# Note: RotatE entity embeddings are 2*KGE_EMBED_DIM (complex representation).
# The server receives 2d-dim vectors per entity instead of d-dim.
# server.py FedVServer and QuestionEncoder are updated accordingly via
# ROTATE_MODE flag below.
...
# RotatE flag — tells server.py to use 2*embed_dim for fusion/MLP output
ROTATE_MODE     = True
