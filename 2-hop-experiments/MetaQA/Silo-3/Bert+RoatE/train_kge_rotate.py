# src/train_kge_rotate.py — Train RotatE independently on each of the 3 silos
# All results saved to logs_rotate/run_<timestamp>/kge_train_log.{txt,json}
...
class KGELogger:
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            f.write("=" * 70 + "\n")
            f.write("  FedV-KGQA  —  KGE Training Log  [RotatE]\n")
...
def train_silo(silo_name, kb_path, shared_entity2id, device, logger):
    print(f"\n{'='*60}")
    print(f"  Training RotatE — {silo_name}")
    print(f"{'='*60}")
