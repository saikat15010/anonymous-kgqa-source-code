# src/train_kge_complex.py — Train ComplEx independently on each of the 3 silos
# All results saved to logs_complex/run_<timestamp>/kge_train_log.{txt,json}
...
class KGELogger:
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            f.write("=" * 70 + "\n")
            f.write("  FedV-KGQA  —  KGE Training Log  [ComplEx]\n")
...
def train_silo(silo_name, kb_path, shared_entity2id, device, logger):
    print(f"\n{'='*60}")
    print(f"  Training ComplEx — {silo_name}")
    print(f"{'='*60}")
    ...
    optimizer = optim.Adam(model.parameters(), lr=KGE_LR,
                           weight_decay=1e-6)   # L2 reg standard for ComplEx
