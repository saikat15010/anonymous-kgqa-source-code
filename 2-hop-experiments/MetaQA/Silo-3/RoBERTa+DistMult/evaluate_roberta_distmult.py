# src/evaluate_roberta_distmult.py — Evaluation for RoBERTa + DistMult
#
# Identical logic to all other evaluate_*.py files.
# Imports from config_roberta_distmult for correct paths and flags.
...
from config_roberta_distmult import USE_TOPIC_ANCHORING
...
def evaluate(server, model_a, model_b, model_c, qa_loader, device,
    ...
    For DistMult: model_{a,b,c}.get_entity_embeddings() returns (N, d) = (N, 256).
    Server fuses them into h_joint ∈ ℝ^(3d) = ℝ^768.
    RoBERTa encoder produces (B, 768) — identical output dim to BERT/DistilBERT.
    """
    server.eval()
    ...
    with torch.no_grad():
        ...
        for questions, topic_ids, answer_ids_batch, candidate_ids in qa_loader:
            ...
            q_embed = server.question_encoder(questions, device)  # (B, 768)
...
def print_results(split_name, overall, per_type=None):
    ...
    print(f"\n{'─'*70}")
    print(f"  {split_name} Results  [RoBERTa + DistMult]")
    print(f"{'─'*70}")
