# src/train_kge_roberta_distmult.py — Train DistMult independently on each of
# the 3 silos for the RoBERTa + DistMult experiment.
#
# KGE training is fully independent of the QA encoder — RoBERTa is not
# involved here at all. This script is logically identical to:
#   train_kge_distmult.py         (Bert+DistMult)
#   train_kge_distilbert_distmult.py (DistilBert+DistMult)
# with only two differences:
#   1. Imports from config_roberta_distmult (different CHECKPOINT_DIR, log dir)
#   2. Log headers label the RoBERTa encoder for traceability
#
# DistMult specifics:
#   - Scoring: φ(h, r, t) = <h, r, t> = Σ h_i · r_i · t_i  (bilinear diagonal)
#   - Entity embeddings ∈ ℝᵈ  (real, same storage as TransE)
#   - L2-normalise entity embeddings after each gradient step
#   - No weight_decay on optimizer (unlike ComplEx which uses 1e-6)
#
# Results saved to:
#   logs_roberta_distmult/run_<timestamp>/kge_train_log.txt
#   logs_roberta_distmult/run_<timestamp>/kge_train_log.json
...
class KGELogger:
    """
    Writes DistMult KGE training results to:
      logs_roberta_distmult/run_<id>/kge_train_log.txt
      logs_roberta_distmult/run_<id>/kge_train_log.json
    ...
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            f.write("=" * 70 + "\n")
            f.write("  FedV-KGQA  —  KGE Training Log  [DistMult + RoBERTa]\n")
...
def train_silo(silo_name, kb_path, shared_entity2id, device, logger):
    """
    Train DistMult on one silo's OWL2-enriched knowledge base.

    DistMult scoring: φ(h, r, t) = <h, r, t> = Σ h_i · r_i · t_i
    Loss: L = max(0, γ − φ(h,r,t⁺) + φ(h,r,t⁻_hard))
    Post-step: entity embeddings L2-normalised to unit sphere.
    """
    print(f"\n{'='*60}")
    print(f"  Training DistMult — {silo_name}  [for RoBERTa+DistMult]")
    print(f"{'='*60}")
    ...
    model     = DistMult(num_entities, num_relations, KGE_EMBED_DIM,
...
def main():
    ...
    # Save run_id so train_fedv_roberta_distmult.py picks up the same log dir
    os.makedirs(CHECKPOINT_DIR, exist_ok=True)
    ...
    # Train DistMult on each silo independently
