# src/train_fedv_roberta_distmult.py — Federated QA training: DistMult + RoBERTa
#
# Compared to train_fedv_distmult.py (Bert+DistMult):
#   - Imports FedVServer from server_roberta_distmult  (RoBERTa encoder)
#   - Imports evaluate/print_results from evaluate_roberta_distmult
#   - Imports config from config_roberta_distmult
#   - Log directory is logs_roberta_distmult/
#   - All hyperparameters, training logic, and gradient handling are identical
#
# Compared to train_fedv_roberta_transe.py (RoBERTa+TransE):
#   - Loads DistMult checkpoints (not TransE)
#   - Same entity dim (256), same joint dim (768), same MLP output (768)
#   - DistMult entity embeddings also L2-normalised after each step (same as TransE)
#
# Results saved to:
#   logs_roberta_distmult/run_<id>/fedv_train_log.txt
#   logs_roberta_distmult/run_<id>/fedv_train_log.json
...
from server_roberta_distmult import FedVServer
from evaluate_roberta_distmult import evaluate, print_results
...
def load_distmult(silo_name, device):
    """Load a pretrained DistMult checkpoint from Phase 1 (KGE training)."""
    ckpt = torch.load(
...
def train():
    ...
    print(f"DistMult entity dim per silo : {KGE_EMBED_DIM}")
    print(f"Joint embedding dim (3d)     : {3 * KGE_EMBED_DIM}")
    print(f"RoBERTa → MLP output         : {3 * KGE_EMBED_DIM}  (matches joint dim)")

    # Reuse run_id created by train_kge_roberta_distmult.py
    ...
    log_dir = os.path.join(os.path.dirname(CHECKPOINT_DIR),
                           "logs_roberta_distmult", run_id)

    config_dict = {
        "kge_model"          : "DistMult",
        "qa_encoder"         : "RoBERTa (roberta-base)",
        "kge_embed_dim"      : KGE_EMBED_DIM,
        "joint_dim"          : 3 * KGE_EMBED_DIM,
        "roberta_dim"        : 768,
        "mlp_output_dim"     : 3 * KGE_EMBED_DIM,
        "qa_lr"              : QA_LR,
        "qa_epochs"          : QA_EPOCHS,
    ...
    # ── Load Phase 1 DistMult checkpoints ─────────────────────────────────────
    print("\nLoading pretrained DistMult models (Phase 1)...")
    ...
    print(f"\nFederated QA training (DistMult + RoBERTa)"
    ...
    for epoch in range(1, QA_EPOCHS + 1):
        ...
        for batch_idx, (questions, topic_ids,
            ...
            # Forward — DistMult: ent_embed.weight ∈ (N, d) = (N, 256)
            loss, _ = server(
                questions, topic_ids,
            ...
            # Backward — ∂L/∂h_joint splits into three (N,256) gradient slices
            loss.backward()
            ...
            # L2-normalise entity embeddings after each update (DistMult standard)
            with torch.no_grad():
    ...
    print("\n" + "=" * 65)
    print("  FINAL TEST RESULTS (DistMult + RoBERTa | 2-hop | 3 silos)")
    print("=" * 65)
