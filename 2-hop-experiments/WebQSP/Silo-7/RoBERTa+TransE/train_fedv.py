# train_fedv.py — Federated QA: RoBERTa+TransE (WebQSP | Client7 — 7 silos)
#
# QA_EPOCHS = 100.
# 7 silo models — fused to joint_dim = 7 * 256 = 1792.
# TransE: L2 normalise entity embeddings after every QA gradient step.
# QA optimisers: Adam, no weight decay.
# Gradient split:
#   A[0:256], B[256:512], C[512:768], D[768:1024],
#   E[1024:1280], F[1280:1536], G[1536:1792]
...
class FedVLogger:
    def __init__(self, log_dir, config_dict):
        ...
        self.data = {"run_id": os.path.basename(log_dir),
                     "kge_model": "TransE", "qa_encoder": "RoBERTa",
        ...
        with open(self.txt_path, "w") as f:
            ...
            f.write("  FedV-KGQA — Federated QA Log"
                    "  [RoBERTa + TransE | WebQSP | Client7 — 7 silos]\n")
...
def train():
    ...
    print(f"Client7 (7 silos) | entity_dim=d={KGE_EMBED_DIM}"
          f" | joint_dim=7×{KGE_EMBED_DIM}={7*KGE_EMBED_DIM}")
    ...
    print("\nLoading pretrained TransE models (7 silos) ...")
    ...
    for epoch in range(1, QA_EPOCHS + 1):
        ...
        for batch_idx, (questions, topic_ids,
            ...
            # TransE: L2 normalise entity embeddings every QA step
            with torch.no_grad():
                for m in all_models:
    ...
    print("\n" + "=" * 60)
    print("  FINAL TEST  [RoBERTa+TransE | WebQSP | Client7 | 7 silos]")
    print("=" * 60)
