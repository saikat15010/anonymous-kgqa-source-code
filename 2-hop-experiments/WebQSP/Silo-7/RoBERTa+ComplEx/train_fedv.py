# train_fedv.py — Federated QA: RoBERTa+ComplEx (WebQSP | Client7 — 7 silos)
#
# QA_EPOCHS = 100.
# 7 silo models — fused to joint_dim = 7 * 2d = 7 * 512 = 3584.
# ComplEx: L2 normalise entity embeddings after every QA gradient step.
# QA optimisers: Adam, NO weight decay (weight_decay=1e-6 is KGE-phase only).
# Gradient split:
#   A[0:512], B[512:1024], C[1024:1536], D[1536:2048],
#   E[2048:2560], F[2560:3072], G[3072:3584]
...
class FedVLogger:
    def __init__(self, log_dir, config_dict):
        ...
        self.data = {"run_id": os.path.basename(log_dir),
                     "kge_model": "ComplEx", "qa_encoder": "RoBERTa",
                     "dataset": "WebQSP", "client": "Client7",
                     "num_silos": 7, "config": config_dict,
                     "train_loss": [], "dev_metrics": [],
                     "test_metrics": None}
        with open(self.txt_path, "w") as f:
            f.write("=" * 70 + "\n")
            f.write("  FedV-KGQA — Federated QA Log"
                    "  [RoBERTa + ComplEx | WebQSP | Client7 — 7 silos]\n")
...
def load_complex(silo_name, device):
    ...
    print(f"  Loaded {silo_name}_complex.pt")
    return model
...
def train():
    ...
    config_dict = {
        "kge_model": "ComplEx", "qa_encoder": "RoBERTa (roberta-base)",
        ...
        "qa_weight_decay": "none (KGE-phase 1e-6 only)",
        "normalise_embeds": True,
    ...
    print("\nBuilding neighbour index from all 7 silos ...")
    ...
    # QA phase: NO weight decay (weight_decay=1e-6 is KGE-phase only)
    ...
    all_optims = [optim_a, optim_b, optim_c, optim_d,
    ...
    for epoch in range(1, QA_EPOCHS + 1):
        ...
        for batch_idx, (questions, topic_ids,
            ...
            # ComplEx: L2 normalise entity embeddings every QA step
            with torch.no_grad():
                for m in all_models:
    ...
    _load(server, all_models, "best", device)
    test_r, test_t = evaluate(server,
                               model_a, model_b, model_c, model_d,
                               model_e, model_f, model_g,
                               test_loader, device, per_type=True)
    print("\n" + "=" * 60)
    print("  FINAL TEST  [RoBERTa+ComplEx | WebQSP | Client7 | 7 silos]")
    print("=" * 60)
    print_results("Test", test_r, test_t)
    logger.log_test(best_epoch, test_r, test_t)


def _save(server, models, tag):
    os.makedirs(CHECKPOINT_DIR, exist_ok=True)
    ckpt = {"server": server.state_dict()}
    labels = ["a", "b", "c", "d", "e", "f", "g"]
    for lbl, m in zip(labels, models):
        ckpt[f"model_{lbl}"] = m.state_dict()
    torch.save(ckpt, os.path.join(CHECKPOINT_DIR, f"fedv_{tag}.pt"))


def _load(server, models, tag, device):
    ckpt = torch.load(os.path.join(CHECKPOINT_DIR, f"fedv_{tag}.pt"),
                      map_location=device)
    server.load_state_dict(ckpt["server"])
    labels = ["a", "b", "c", "d", "e", "f", "g"]
    for lbl, m in zip(labels, models):
        m.load_state_dict(ckpt[f"model_{lbl}"])
