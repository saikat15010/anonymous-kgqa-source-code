# train_kge.py — Train TransE independently on each of the 7 silos
# WebQSP | Client7 | BERT+TransE
#
...
# Checkpoint names: silo_*_transe.pt  (7 checkpoints: a/b/c/d/e/f/g)
...
def train_silo(silo_name, kb_path, shared_entity2id, device, logger):
    ...
    print(f"  Training TransE — {silo_name.upper()}"
          f"  [WebQSP | Client7 | BERT+TransE]")
    print(f"  entity_dim=d={KGE_EMBED_DIM}  joint_dim=7×{KGE_EMBED_DIM}={7*KGE_EMBED_DIM}")
    print(f"{'='*60}")
...
def main():
    ...
    config_dict = {
        ...
        "joint_dim": 7 * KGE_EMBED_DIM, "kge_lr": KGE_LR,
    ...
    # Build shared entity vocabulary from all 7 silos
