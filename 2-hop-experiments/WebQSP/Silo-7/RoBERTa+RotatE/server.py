# server.py — Federated server for WebQSP Client7 RoBERTa+RotatE (7 silos)
#
# RotatE entity_dim = 2d = 512
# Client7: joint_dim = 7 * 2d = 7 * 512 = 3584
# MLP: 768 → 512 → 3584
#
# RoBERTa: NO token_type_ids, uses RobertaTokenizer/RobertaModel.
# MLP output (3584) doubles vs RoBERTa+TransE/DistMult (1792).
...
class QuestionEncoder(nn.Module):
    """
    RoBERTa [CLS] → MLP → q_embed in R^3584. RoBERTa frozen.
    MLP: 768 → 512 → 3584.
...
class FedVServer(nn.Module):
    """Central server — WebQSP Client7 RoBERTa+RotatE (7 silos)."""
