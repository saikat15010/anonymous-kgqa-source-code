# train_fedv.py — Federated QA: RoBERTa+RotatE (WebQSP | Client7 — 7 silos)
#
# QA_EPOCHS = 100.
# 7 silo models — fused to joint_dim = 7 * 2d = 7 * 512 = 3584.
# RotatE: L2 normalise entity embeddings after every QA gradient step.
...
class FedVLogger:
    def __init__(self, log_dir, config_dict):
        ...
        self.data = {"run_id": os.path.basename(log_dir),
                     "kge_model": "RotatE", "qa_encoder": "RoBERTa",
        ...
        with open(self.txt_path, "w") as f:
            ...
            f.write("  FedV-KGQA — Federated QA Log"
                    "  [RoBERTa + RotatE | WebQSP | Client7 — 7 silos]\n")
...
def load_rotate(silo_name, device):
    ...
    print(f"  Loaded {silo_name}_rotate.pt")
    return model
...
def train():
    ...
    config_dict = {
        "kge_model": "RotatE", "qa_encoder": "RoBERTa (roberta-base)",
        ...
        "normalise_embeds": True, "qa_weight_decay": "none",
    ...
    for epoch in range(1, QA_EPOCHS + 1):
        ...
        for batch_idx, (questions, topic_ids,
            ...
            # RotatE: L2 normalise entity embeddings every QA step
            with torch.no_grad():
                for m in all_models:
    ...
    print("\n" + "=" * 60)
    print("  FINAL TEST  [RoBERTa+RotatE | WebQSP | Client7 | 7 silos]")
    print("=" * 60)
