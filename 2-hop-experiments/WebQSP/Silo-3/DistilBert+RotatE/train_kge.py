# train_kge.py — Train RotatE independently on each of the 3 silos
# WebQSP | Client3 | DistilBERT+RotatE
#
# KGE training is encoder-agnostic — DistilBERT is not involved here.
# RotatE: Adam, no weight decay. L2 normalise entity embeddings every step.
# Checkpoint names: silo_*_rotate.pt
...
class KGELogger:
    def __init__(self, log_dir, config_dict):
        ...
        self.data = {"run_id": os.path.basename(log_dir),
                     ...
                     "num_silos": 3, "config": config_dict, "silos": {}}
        with open(self.txt_path, "w") as f:
            f.write("=" * 70 + "\n")
            f.write("  FedV-KGQA — KGE Training Log"
                    "  [RotatE | WebQSP | Client3 | DistilBERT]\n")
    ...
    def finish_all(self):
        ...
        with open(self.txt_path, "a") as f:
            f.write(f"  All 3 silos complete. Total: {total/60:.1f} min\n")
        self._flush()
...
def train_silo(silo_name, kb_path, shared_entity2id, device, logger):
    print(f"\n{'='*60}")
    print(f"  Training RotatE — {silo_name.upper()}"
          f"  [WebQSP | Client3 | DistilBERT+RotatE]")
    ...
    model     = RotatE(num_entities, num_relations,
    ...
    for epoch in range(1, KGE_EPOCHS + 1):
        ...
        for h, r, t_pos in loader:
            ...
            loss.backward()
            optimizer.step()
            # RotatE: L2 normalise entity embeddings every step
            with torch.no_grad():
...
def main():
    ...
    config_dict = {
        ...
        "kge_lr": KGE_LR, "kge_epochs": KGE_EPOCHS,
        "kge_batch_size": KGE_BATCH_SIZE, "kge_neg_samples": KGE_NEG_SAMPLES,
        "kge_margin": KGE_MARGIN, "normalise_embeds": True,
