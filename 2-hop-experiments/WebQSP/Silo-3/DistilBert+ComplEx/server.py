# server.py — Federated server for WebQSP Client3 DistilBERT+ComplEx (3 silos)
#
# ComplEx entity_dim = 2d = 512  →  joint_dim = 3 × 512 = 1536
# MLP: 768 → 512 → 1536  (same architecture as DistilBERT+RotatE)
#
# DistilBERT differences vs BERT:
#   1. DistilBertTokenizer / DistilBertModel
#   2. NO token_type_ids — DistilBERT raises error if passed
#   3. [CLS] still at index 0 in last_hidden_state
#   4. Same 768-dim output — MLP and joint_dim unchanged vs RotatE
...
JOINT_DIM  = 3 * ENTITY_DIM       # 3 × 512 = 1536


class QuestionEncoder(nn.Module):
    """
    DistilBERT [CLS] → MLP → q_embed in R^1536. DistilBERT frozen.
    MLP: 768 → 512 → 1536.
    No token_type_ids — DistilBERT does not support them.
    """
    ...
    def __init__(self):
        ...
        for h_dim in MLP_HIDDEN_DIMS:
            ...
            in_dim = h_dim
        layers.append(nn.Linear(in_dim, JOINT_DIM))   # → 1536
...
class FedVServer(nn.Module):
    """Central server — WebQSP Client3 DistilBERT+ComplEx (3 silos)."""
