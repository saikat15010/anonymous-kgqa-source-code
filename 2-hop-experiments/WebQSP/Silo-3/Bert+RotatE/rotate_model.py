# rotate_model.py — RotatE Knowledge Graph Embedding model
# WebQSP | Client3 | 3 silos | BERT+RotatE
#
# Entity h, t in C^d stored as 2d reals [Re(h)||Im(h)].
# Relation r = phase rotation in [-π, π]^d  (d reals, NOT 2d).
# Scoring: phi(h,r,t) = -||h ∘ e^(ir) - t||
# L2 normalisation of entity embeddings after every gradient step.
#
# entity_dim = 2d = 512,  joint_dim = 3×512 = 1536  for Client3.

import torch
import torch.nn as nn
import torch.nn.functional as F
import math


class RotatE(nn.Module):
    """
    RotatE: entities in C^d stored as [Re||Im] in R^(2d).
    Relations are phase angles in R^d.

    phi(h,r,t) = -||h ∘ e^(ir) - t||_2

    entity_dim = 2d = 512,  joint_dim = 3×2d = 1536  (Client3).
    """

    def __init__(self, num_entities: int, num_relations: int,
                 embed_dim: int, norm: int = 2):
        super().__init__()
        self.embed_dim = embed_dim   # d
        self.norm      = norm

        self.ent_embed = nn.Embedding(num_entities, 2 * embed_dim)
        self.rel_embed = nn.Embedding(num_relations, embed_dim)  # d phase angles

        self._init_weights()

    def _init_weights(self):
        nn.init.xavier_uniform_(self.ent_embed.weight)
        nn.init.uniform_(self.rel_embed.weight, -math.pi, math.pi)
        with torch.no_grad():
            self.ent_embed.weight.data = F.normalize(
                self.ent_embed.weight.data, p=2, dim=-1)

    def _complex_rotate(self, h_ids, r_ids, t_ids):
        h = self.ent_embed(h_ids)
        t = self.ent_embed(t_ids)
        r = self.rel_embed(r_ids)
        d = self.embed_dim
        h_re, h_im = h[:, :d], h[:, d:]
        t_re, t_im = t[:, :d], t[:, d:]
        r_re, r_im = torch.cos(r), torch.sin(r)
        rot_re = h_re * r_re - h_im * r_im
        rot_im = h_re * r_im + h_im * r_re
        return torch.cat([rot_re - t_re, rot_im - t_im], dim=-1)

    def score(self, h_ids, r_ids, t_ids):
        return -torch.norm(self._complex_rotate(h_ids, r_ids, t_ids),
                           p=self.norm, dim=-1)

    def margin_ranking_loss(self, h_ids, r_ids, t_pos_ids,
                            t_neg_ids, margin=1.0):
        pos_scores = self.score(h_ids, r_ids, t_pos_ids)
        B, K = t_neg_ids.shape
        h_exp      = h_ids.unsqueeze(1).expand(B, K).reshape(-1)
        r_exp      = r_ids.unsqueeze(1).expand(B, K).reshape(-1)
        t_neg_flat = t_neg_ids.reshape(-1)
        neg_scores = self.score(h_exp, r_exp, t_neg_flat).reshape(B, K)
        hard_neg, _ = neg_scores.max(dim=1)
        return F.relu(margin - pos_scores + hard_neg).mean()

    def get_entity_embeddings(self, entity_ids=None):
        """Returns 2d-dim [Re||Im] embeddings for the server."""
        if entity_ids is None:
            return self.ent_embed.weight
        return self.ent_embed(entity_ids)
