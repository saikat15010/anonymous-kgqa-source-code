# train_fedv.py — Federated QA training BERT+ComplEx (WebQSP | Client3 — 3 silos)
#
...
# Gradient split: Silo A [0:512], Silo B [512:1024], Silo C [1024:1536].
...
class FedVLogger:
    def __init__(self, log_dir, config_dict):
        ...
        self.data = {"run_id": os.path.basename(log_dir),
                     ...
                     "dataset": "WebQSP", "client": "Client3",
                     "num_silos": 3, "config": config_dict,
                     "train_loss": [], "dev_metrics": [],
                     "test_metrics": None}
        with open(self.txt_path, "w") as f:
            f.write("=" * 70 + "\n")
            f.write("  FedV-KGQA — Federated QA Log"
                    "  [BERT + ComplEx | WebQSP | Client3 — 3 silos]\n")
...
def train():
    ...
    print(f"Client3 | entity_dim=2d={2*KGE_EMBED_DIM}"
          f" | joint_dim=3×{2*KGE_EMBED_DIM}={3*2*KGE_EMBED_DIM}")
    ...
    config_dict = {
        ...
        "dataset": "WebQSP", "client": "Client3", "num_silos": 3,
        "kge_embed_dim": KGE_EMBED_DIM,
        "entity_dim": 2 * KGE_EMBED_DIM,
        "relation_dim": 2 * KGE_EMBED_DIM,
        "joint_dim": 3 * 2 * KGE_EMBED_DIM,
        "bert_dim": 768, "qa_lr": QA_LR, "qa_epochs": QA_EPOCHS,
    ...
    print("\nLoading pretrained ComplEx models ...")
    ...
    for epoch in range(1, QA_EPOCHS + 1):
        ...
        if epoch % 2 == 0 or epoch == QA_EPOCHS:
            ...
            dev_r, dev_t = evaluate(server, model_a, model_b, model_c,
    ...
    test_r, test_t = evaluate(server, model_a, model_b, model_c,
                               test_loader, device, per_type=True)
    print("\n" + "=" * 60)
    print("  FINAL TEST  [BERT+ComplEx | WebQSP | Client3 | 3 silos]")
    print("=" * 60)
...
def _save(server, ma, mb, mc, tag):
    ...
    torch.save({"server": server.state_dict(), "model_a": ma.state_dict(),
                "model_b": mb.state_dict(), "model_c": mc.state_dict()},
