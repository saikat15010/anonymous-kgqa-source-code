...
# WebQSP | Client3 | DistilBERT+TransE
#
# KGE training is encoder-agnostic — DistilBERT is not involved here.
# Identical to BERT+TransE train_kge.py except CHECKPOINT_DIR and log labels.
# TransE: Adam, no weight decay. L2 normalise entity embeddings every step.
# Checkpoint names: silo_*_transe.pt
    ...
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            ...
            f.write("  FedV-KGQA — KGE Training Log"
                    "  [TransE | WebQSP | Client3 | DistilBERT]\n")
...
def train_silo(silo_name, kb_path, shared_entity2id, device, logger):
    ...
    print(f"  Training TransE — {silo_name.upper()}"
          f"  [WebQSP | Client3 | DistilBERT+TransE]")
    print(f"  entity_dim=d={KGE_EMBED_DIM}  joint_dim={3*KGE_EMBED_DIM}")
    print(f"{'='*60}")
...
def main():
    ...
    config_dict = {
        ...
        "joint_dim": 3 * KGE_EMBED_DIM, "kge_lr": KGE_LR,
