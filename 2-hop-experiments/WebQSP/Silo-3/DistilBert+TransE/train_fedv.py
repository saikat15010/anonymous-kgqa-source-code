# train_fedv.py — Federated QA: DistilBERT+TransE (WebQSP | Client3 — 3 silos)
#
...
# Gradient split: Silo A [0:256], Silo B [256:512], Silo C [512:768].
    ...
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            ...
            f.write("  FedV-KGQA — Federated QA Log"
                    "  [DistilBERT + TransE | WebQSP | Client3 — 3 silos]\n")
...
def train():
    ...
    print(f"Client3 | entity_dim=d={KGE_EMBED_DIM}"
          f" | joint_dim=3×{KGE_EMBED_DIM}={3*KGE_EMBED_DIM}")
    print(f"QA_EPOCHS = {QA_EPOCHS}  (DistilBERT experiments use 100)")
    ...
    config_dict = {
        "kge_model": "TransE", "qa_encoder": "DistilBERT (distilbert-base-uncased)",
        "dataset": "WebQSP", "client": "Client3", "num_silos": 3,
        "kge_embed_dim": KGE_EMBED_DIM, "entity_dim": KGE_EMBED_DIM,
        "joint_dim": 3 * KGE_EMBED_DIM, "distilbert_dim": 768,
    ...
    print("\n" + "=" * 60)
    print("  FINAL TEST  [DistilBERT+TransE | WebQSP | Client3 | 3 silos]")
    print("=" * 60)
