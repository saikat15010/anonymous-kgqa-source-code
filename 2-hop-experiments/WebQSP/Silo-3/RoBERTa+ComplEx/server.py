# server.py — Federated server for WebQSP Client3 RoBERTa+ComplEx (3 silos)
#
# ComplEx entity_dim = 2d = 512  →  joint_dim = 3 × 512 = 1536
# MLP: 768 → 512 → 1536  (same architecture as RoBERTa+RotatE)
#
# RoBERTa: NO token_type_ids, uses RobertaTokenizer/RobertaModel.
# Same 768-dim output — MLP output (1536) same as RotatE, differs from
# TransE/DistMult (768).
...
class QuestionEncoder(nn.Module):
    """
    RoBERTa [CLS] → MLP → q_embed in R^1536. RoBERTa frozen.
    MLP: 768 → 512 → 1536.
...
class FedVServer(nn.Module):
    """Central server — WebQSP Client3 RoBERTa+ComplEx (3 silos)."""
