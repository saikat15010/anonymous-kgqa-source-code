# train_kge.py — Train TransE independently on each of the 3 silos
# WebQSP | Client3 | BERT+TransE
#
# KGE training is encoder-agnostic — BERT is not involved here.
# TransE: standard Adam optimiser, no weight decay.
# Entity embeddings are L2-normalised after every gradient step.
# Only entity embeddings (NOT relation embeddings) are saved for the server.
...
def sample_negatives(batch_size: int, num_entities: int,
                     num_neg: int, device: torch.device) -> torch.Tensor:
    return torch.randint(0, num_entities,
                         (batch_size, num_neg), device=device)
...
class KGELogger:
    def __init__(self, log_dir: str, config_dict: dict):
        ...
        with open(self.txt_path, "w") as f:
            ...
            f.write("  FedV-KGQA — KGE Training Log"
                    "  [TransE | WebQSP | Client3 — 3 silos | BERT]\n")
    ...
    def start_silo(self, silo_name: str, num_entities: int,
    ...
    def finish_silo(self, silo_name: str, ckpt_path: str):
        elapsed = time.time() - self._silo_start
...
def train_silo(silo_name: str, kb_path: str,
               shared_entity2id: dict, device: torch.device,
               logger: KGELogger):
    print(f"\n{'='*60}")
    print(f"  Training TransE — {silo_name.upper()}  "
          f"[WebQSP | Client3 | BERT+TransE]")
    print(f"{'='*60}")
    ...
    # TransE: standard Adam, no weight decay
    ...
    for epoch in range(1, KGE_EPOCHS + 1):
        ...
        for h, r, t_pos in loader:
            ...
            loss.backward()
            optimizer.step()
            # L2-normalise entity embeddings after every step
            with torch.no_grad():
...
def main():
    ...
    # Save run ID for train_fedv.py to pick up
    ...
    # Build shared entity vocabulary across all 3 silos
    print("\nBuilding shared entity vocabulary ...")
    ...
    # Save shared vocab for train_fedv.py
