# train_fedv.py — Federated QA training BERT + TransE (WebQSP | Client3 — 3 silos)
#
# QA-phase optimisers use NO weight decay.
# Gradient splitting: server sends each silo only its d-column slice
#   Silo A: columns [0   : 256]
#   Silo B: columns [256 : 512]
#   Silo C: columns [512 : 768]
...
class FedVLogger:
    def __init__(self, log_dir: str, config_dict: dict):
        ...
        with open(self.txt_path, "w") as f:
            ...
            f.write("  FedV-KGQA — Federated QA Training Log"
                    "  [BERT + TransE | WebQSP | Client3 — 3 silos]\n")
    ...
    def log_train_epoch(self, epoch: int, avg_loss: float):
    ...
    def log_dev(self, epoch: int, train_loss: float,
                overall: dict, per_type: dict):
        self.data["dev_metrics"].append({
            "epoch": epoch,
            "train_avg_loss": round(train_loss, 6),
    ...
    def log_test(self, best_epoch: int, overall: dict, per_type: dict):
        ...
        with open(self.txt_path, "a") as f:
            f.write("=" * 70 + "\n")
            f.write(f"  FINAL TEST RESULTS (best epoch {best_epoch})\n")
            f.write("=" * 70 + "\n")
            f.write(_fmt_overall(overall))
            f.write(_fmt_per_type(per_type))
            f.write("=" * 70 + "\n")
...
def _fmt_overall(r: dict) -> str:
    return (f"  Overall | MRR:{r['mrr']:.4f} H@1:{r['hits@1']:.4f}"
            f" H@3:{r['hits@3']:.4f} H@5:{r['hits@5']:.4f}"
            f" H@10:{r['hits@10']:.4f} N:{r['total']}\n")


def _fmt_per_type(per_type: dict) -> str:
    lines = "  Per Type:\n"
    for atype in ["place_birth", "nationality", "profession",
        ...
        if atype not in per_type:
            continue
        r = per_type[atype]
        lines += (f"    {atype:<14} MRR:{r['mrr']:.4f}"
                  f" H@1:{r['hits@1']:.4f} N:{r['total']}\n")
    return lines


def make_collate_fn(candidate_map: dict):
    def collate_fn(batch):
        questions  = [item[0] for item in batch]
        topic_ids  = torch.tensor([item[1] for item in batch],
                                   dtype=torch.long)
...
def load_transe(silo_name: str, device: torch.device) -> TransE:
    ckpt = torch.load(
        os.path.join(CHECKPOINT_DIR, f"{silo_name}_transe.pt"),
        map_location=device)
    model = TransE(num_entities=ckpt["num_entities"],
...
def train():
    ...
    print(f"Client3 — 3 silos | entity_dim=d={KGE_EMBED_DIM}"
          f" | joint_dim=3×{KGE_EMBED_DIM}={3*KGE_EMBED_DIM}")

    # Reuse the run ID created by train_kge.py
    ...
    config_dict = {
        "kge_model"     : "TransE",
        "qa_encoder"    : "BERT (bert-base-uncased)",
        "dataset"       : "WebQSP",
        "client"        : "Client3",
        "num_silos"     : 3,
        "kge_embed_dim" : KGE_EMBED_DIM,
        "entity_dim"    : KGE_EMBED_DIM,
    ...
    # Load shared vocabulary saved by train_kge.py
    ...
    # 2-hop candidate precomputation (larger caps for WebQSP)
    print("\nBuilding neighbour index (3 silos) ...")
    ...
    # Load pretrained TransE models
    print("\nLoading pretrained TransE models (3 silos) ...")
    ...
    # QA-phase: no weight_decay (weight_decay=1e-6 is ComplEx KGE-phase only)
    ...
    print(f"\nFederated QA training — {QA_EPOCHS} epochs | "
    ...
    for epoch in range(1, QA_EPOCHS + 1):
        ...
        for batch_idx, (questions, topic_ids,
            ...
            for opt in [optim_a, optim_b, optim_c]:
                opt.zero_grad()
            ...
            for m in [model_a, model_b, model_c]:
                torch.nn.utils.clip_grad_norm_(
                    m.ent_embed.parameters(), 1.0)

            server_optimizer.step()
            for opt in [optim_a, optim_b, optim_c]:
                opt.step()

            # TransE: L2-normalise entity embeddings in all 3 silos
            with torch.no_grad():
                for m in [model_a, model_b, model_c]:
        ...
        if epoch % 2 == 0 or epoch == QA_EPOCHS:
            ...
            dev_results, dev_type = evaluate(
                server, model_a, model_b, model_c,
                dev_loader, device, per_type=True)
            print_results("Dev", dev_results, dev_type)
            logger.log_dev(epoch, avg_loss, dev_results, dev_type)
    ...
    test_results, test_type = evaluate(
        server, model_a, model_b, model_c,
    ...
    print("  FINAL TEST RESULTS"
          "  [BERT+TransE | WebQSP | Client3 | 3 silos]")
    print("=" * 60)
    print_results("Test", test_results, test_type)
    print("=" * 60)

    logger.log_test(best_epoch, test_results, test_type)


def _save(server, ma, mb, mc, tag: str):
...
def _load(server, ma, mb, mc, tag: str, device: torch.device):
    ckpt = torch.load(
        os.path.join(CHECKPOINT_DIR, f"fedv_{tag}.pt"),
        map_location=device)
