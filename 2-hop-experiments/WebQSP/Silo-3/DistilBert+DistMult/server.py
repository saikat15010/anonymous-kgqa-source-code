# server.py — Federated server for WebQSP Client3 DistilBERT+DistMult (3 silos)
#
# DistMult entity_dim = d = 256  →  joint_dim = 3 * d = 768
# MLP: 768 → 512 → 768
#
# DistilBERT differences vs BERT (same as DistilBERT+TransE server):
...
#   4. Same 768-dim output → joint_dim and MLP unchanged
...
class QuestionEncoder(nn.Module):
    ...
    MLP: 768 → 512 → 768.
    No token_type_ids — DistilBERT raises an error if passed.
    """
    ...
    def forward(self, questions, device):
        # token_type_ids deliberately omitted — DistilBERT does not support them
        enc = self.tokenizer(
...
class FedVServer(nn.Module):
    """Central server — WebQSP Client3 DistilBERT+DistMult (3 silos)."""
