# evaluate.py — Evaluation for WebQSP Client3 BERT+DistMult (3 silos)
# Metrics: MRR, Hits@1, Hits@3, Hits@5, Hits@10
# Per-type breakdown uses WebQSP answer categories.
...
def detect_answer_type(question: str) -> str:
    ...
    if any(w in q for w in ["spouse", "married", "husband", "wife",
                             "partner", "child", "children", "son",
                             "daughter", "parent", "father", "mother"]):
        return "person"
    ...
    if any(w in q for w in ["president", "leader", "prime minister",
                             ...
                             "direct", "director", "directed by",
                             "star", "actor", "actress", "cast"]):
        return "person"
...
def print_results(split_name, overall, per_type=None):
    ...
    print(f"  {split_name} Results"
          f"  [BERT + DistMult | WebQSP | Client3 — 3 silos]")
    print(f"{'─'*70}")
