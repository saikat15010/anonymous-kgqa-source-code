# server.py — Federated server for WebQSP Client5 RoBERTa+RotatE (5 silos)
#
# RotatE entity_dim = 2d = 512
# Client5: joint_dim = 5 * 2d = 5 * 512 = 2560
# MLP: 768 → 512 → 2560
#
# RoBERTa: NO token_type_ids, uses RobertaTokenizer/RobertaModel.
# Same 768-dim output — MLP output (2560) doubles vs RoBERTa+TransE/DistMult.
...
JOINT_DIM  = NUM_SILOS * ENTITY_DIM  # 5 × 512 = 2560


class QuestionEncoder(nn.Module):
    """
    RoBERTa [CLS] → MLP → q_embed in R^2560. RoBERTa frozen.
    MLP: 768 → 512 → 2560.
    ...
    def __init__(self):
        ...
        for h_dim in MLP_HIDDEN_DIMS:
            ...
            in_dim = h_dim
        layers.append(nn.Linear(in_dim, JOINT_DIM))   # → 2560
...
class FedVServer(nn.Module):
    """Central server — WebQSP Client5 RoBERTa+RotatE (5 silos)."""
