...
#
# WebQSP entities are human-readable surface strings (e.g. "Barack Obama"),
# identical format to MetaQA and PathQuestion.
...
def build_index(kb_path: str):
    """
    Read a silo KB file and return:
      triples       : list of (h, r, t) string tuples
      entity2id     : dict {entity_name -> int id}
      relation2id   : dict {relation_name -> int id}
    """
...
def build_shared_entity_index(*entity2id_dicts):
    """
    Merge entity vocabularies from all silos into one shared index.
    This is the defining operation of vertical federation:
    all silos share the same entity IDs, only relations are disjoint.
    """
    all_entities = set()
...
def build_neighbor_index(kb_paths: list,
    ...
    """
    Build a combined adjacency map from all silos.
    neighbors[entity_id] = list of neighbouring entity IDs (capped).
    Used for 2-hop candidate expansion during QA.
    """
...
def precompute_candidates(entity_ids: list,
    ...
    """
    For each entity, precompute its 2-hop candidate set.
    candidate_map[entity_id] = LongTensor of candidate entity IDs.

    Caps are higher than MetaQA/PQ2H because WebQSP entities can have
    very high degree (hub cities, famous people with 1000+ connections).
    """
...
class KGEDataset(Dataset):
    """Tab-separated KB triples mapped to shared entity IDs."""
...
def parse_qa_file(qa_path: str) -> list:
    """
    Read a QA file and return list of (question, topic_entity, [answers]).
    Multiple rows with the same question are merged into one sample
...
class QADataset(Dataset):
    """QA dataset with entity ID resolution."""
