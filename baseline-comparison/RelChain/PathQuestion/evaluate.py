# evaluate.py — Evaluation for Adapted Relational Chain Reasoning (VFL)
# Uses avg pooling (d-dim), combined entity + chain scoring, no anchoring.
...
def evaluate(server, model_a, model_b, model_c, qa_loader, device,
    ...
    with torch.no_grad():
        ...
        h_joint = server.fuse(h_a, h_b, h_c)  # (N, d)
        ...
        for questions, topic_ids, answer_ids_batch, candidate_ids in qa_loader:
            ...
            # Combined scoring: entity sim + chain reasoning
            sim = server.score_with_chain(q_final, h_joint, candidate_ids)
