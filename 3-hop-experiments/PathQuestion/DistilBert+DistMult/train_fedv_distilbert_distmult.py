# train_fedv_distilbert_distmult.py — Federated QA training DistMult+DistilBERT (PQ3H)
# All results saved to logs_distilbert_distmult/run_<timestamp>/fedv_train_log.{txt,json}
#
...
#   - Log headers reflect PQ3H / 3-hop
...
from config_distilbert_distmult import (
    ...
    CANDIDATE_HOP3_CAP, NUM_HOPS
)
from dataset import QADataset, build_neighbor_index, precompute_candidates
from distmult_model import DistMult
from server_distilbert import FedVServer
from evaluate_distilbert import evaluate, print_results
    ...
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            ...
            f.write(f"  FedV-KGQA — Federated QA Training Log"
                    f"  [DistMult + DistilBERT | PQ3H | {NUM_HOPS}-hop]\n")
...
def train():
    ...
    print(f"DistilBERT + DistMult | PQ3H | {NUM_HOPS}-hop"
          f" | joint_dim={3*KGE_EMBED_DIM}")
    ...
    print("\nLoading pretrained DistMult models ...")
    ...
    print(f"\nFederated QA training (DistilBERT + DistMult | PQ3H {NUM_HOPS}-hop)"
    ...
    for epoch in range(1, QA_EPOCHS + 1):
        ...
        for batch_idx, (questions, topic_ids,
            ...
            # L2 normalise entity embeddings every step (preserves 2-hop behavior)
            with torch.no_grad():
    ...
    print(f"  FINAL TEST RESULTS"
          f" (DistilBERT + DistMult | PQ3H | {NUM_HOPS}-hop | 3 silos)")
    print("=" * 60)
