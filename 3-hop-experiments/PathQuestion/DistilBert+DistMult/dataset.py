# dataset.py — Entity/relation indexing + QA/KGE data loaders (PQ3H)
#
# Same format as PQ2H:
#   KB delimiter: TAB (\t)
#   QA format: question\ttopic_entity\tanswer (3 columns, single answer per line)
# Extended with 3-hop candidate filtering support.
...
def build_neighbor_index(kb_paths, shared_entity2id, max_neighbors=100):
    """Bidirectional neighbor index (tab-separated KB)."""
...
def precompute_candidates(entity_ids, neighbor_index,
    ...
    """
    Precompute candidate sets — supports 2-hop and 3-hop expansion.

    num_hops=2: topic → hop1 → hop2
    num_hops=3: topic → hop1 → hop2 → hop3
    """
...
def parse_qa_file(qa_path):
    """Parse PQ QA file: question\\ttopic_entity\\tanswer (one answer per line)."""
    samples = []
