# train_fedv.py — Federated QA training with TransE + BERT (PQ3H — 3-hop)
...
class FedVLogger:
    def __init__(self, log_dir, config_dict):
        ...
        self.data = {
            "run_id": os.path.basename(log_dir), "kge_model": "TransE",
            "qa_encoder": "BERT", "dataset": "PQ3H", "num_hops": NUM_HOPS,
            "config": config_dict, "train_loss": [],
            "dev_metrics": [], "test_metrics": None}
        with open(self.txt_path, "w") as f:
            f.write("=" * 70 + "\n")
            f.write(f"  FedV-KGQA — Federated QA Log"
                    f"  [TransE + BERT | PQ3H | {NUM_HOPS}-hop]\n")
...
def train():
    ...
    config_dict = {
        "kge_model": "TransE", "qa_encoder": "BERT", "dataset": "PQ3H",
        "num_hops": NUM_HOPS, "kge_embed_dim": KGE_EMBED_DIM,
        "joint_dim": 3 * KGE_EMBED_DIM, "qa_lr": QA_LR,
        ...
        "max_neighbors": MAX_NEIGHBORS, "seed": SEED, "device": str(device)}
    ...
    print(f"\nBuilding {NUM_HOPS}-hop neighbor index ...")
    ...
    candidate_map = precompute_candidates(
        ...
        hop3_cap=CANDIDATE_HOP3_CAP, num_hops=NUM_HOPS)
    ...
    print(f"\nFederated QA ({NUM_HOPS}-hop) — {QA_EPOCHS} epochs | "
    ...
    print("\n" + "=" * 60)
    print(f"  FINAL TEST (TransE + BERT | PQ3H | {NUM_HOPS}-hop | 3 silos)")
    print("=" * 60)
