# src/train_kge_distilbert_complex.py — Train ComplEx independently on each of
# the 3 silos for the DistilBERT + ComplEx experiment.
#
# KGE training is fully independent of the QA encoder — DistilBERT is not
# involved here at all. This script is identical in logic to train_kge_complex.py
# (Bert+ComplEx) with only two differences:
#   1. Imports from config_distilbert_complex (different CHECKPOINT_DIR, log dir)
#   2. Log header labels reflect the DistilBERT encoder for traceability
#
# Key ComplEx properties vs other KGE models:
#   - Entity AND relation embeddings are 2d-dim complex [re|im] — same storage
#     as RotatE but different scoring (Hermitian dot product vs rotation)
#   - Adam optimizer uses weight_decay=1e-6 (L2 regularisation, standard for ComplEx)
#   - Entity embeddings L2-normalised after each gradient step (same as TransE/DistMult)
#   - No unit-modulus constraint on relations (unlike RotatE)
#   - Handles anti-symmetric relations via imaginary components
#
# Results saved to:
#   logs_distilbert_complex/run_<timestamp>/kge_train_log.txt
#   logs_distilbert_complex/run_<timestamp>/kge_train_log.json
...
class KGELogger:
    """
    Writes ComplEx KGE training results to:
      logs_distilbert_complex/run_<id>/kge_train_log.txt
      logs_distilbert_complex/run_<id>/kge_train_log.json
    ...
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            f.write("=" * 70 + "\n")
            f.write("  FedV-KGQA  —  KGE Training Log  [ComplEx + DistilBERT]\n")
    ...
    def start_silo(self, silo_name, num_entities, num_relations, num_triples):
        ...
        with open(self.txt_path, "a") as f:
            ...
            f.write(f"  Entity emb dim  : {ENTITY_DIM:>10,}  "
                    f"(2 × {KGE_EMBED_DIM} complex [re|im])\n")
            f.write("-" * 70 + "\n")
...
def train_silo(silo_name, kb_path, shared_entity2id, device, logger):
    """
    Train ComplEx on one silo's enriched knowledge base.
    ...
    Optimizer: Adam with weight_decay=1e-6 (L2 regularisation, standard for ComplEx)
    After each step: entity embeddings L2-normalised to unit sphere.
    Relation embeddings are unconstrained (unlike RotatE which uses unit modulus).
    """
    print(f"\n{'='*60}")
    print(f"  Training ComplEx — {silo_name}  [for DistilBERT+ComplEx]")
    print(f"  Entity embed dim: {ENTITY_DIM} (= 2 × {KGE_EMBED_DIM} complex [re|im])")
    print(f"{'='*60}")
    ...
    for epoch in range(1, KGE_EPOCHS + 1):
        ...
        for h, r, t_pos in loader:
            ...
            # (same as TransE and DistMult; unlike RotatE which doesn't need this)
            with torch.no_grad():
...
def main():
    ...
    # Save run_id so train_fedv_distilbert_complex.py picks up the same log dir
    os.makedirs(CHECKPOINT_DIR, exist_ok=True)
    ...
    # Train ComplEx on each silo independently
