# train_fedv_distilbert_complex.py — Federated QA training DistilBERT+ComplEx
# MetaQA | 3-hop | 3 silos
#
# CHANGES FROM 2-HOP VERSION:
#   - Imports NUM_HOPS and CANDIDATE_HOP3_CAP from config
#   - Passes num_hops and hop3_cap to precompute_candidates()
#   - Log headers reflect "3-hop"
...
from config_distilbert_complex import (
    ...
    CANDIDATE_HOP1_CAP, CANDIDATE_HOP2_CAP, CANDIDATE_HOP3_CAP,
    NUM_HOPS, ENTITY_DIM, JOINT_DIM
)
...
class FedVLogger:
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            f.write("=" * 70 + "\n")
            f.write(f"  FedV-KGQA — Federated QA Training Log"
                    f"  [DistilBERT + ComplEx | {NUM_HOPS}-hop]\n")
...
def train():
    ...
    print(f"DistilBERT + ComplEx | {NUM_HOPS}-hop | entity_dim={ENTITY_DIM}"
    ...
    config_dict = {
        ...
        "candidate_hop3_cap" : CANDIDATE_HOP3_CAP,
    ...
    print(f"\nBuilding neighbor index ({NUM_HOPS}-hop) ...")
    neighbor_index = build_neighbor_index(
        [SILO_A_KB, SILO_B_KB, SILO_C_KB],
        shared_entity2id, max_neighbors=MAX_NEIGHBORS
    )
    print(f"Precomputing {NUM_HOPS}-hop candidates ...")
    candidate_map = precompute_candidates(
        list(shared_entity2id.values()), neighbor_index,
        hop1_cap=CANDIDATE_HOP1_CAP,
        hop2_cap=CANDIDATE_HOP2_CAP,
        hop3_cap=CANDIDATE_HOP3_CAP,
        num_hops=NUM_HOPS
    )
    ...
    print(f"\nBuilding QA datasets ({NUM_HOPS}-hop) ...")
    ...
    print(f"\nFederated QA training (DistilBERT + ComplEx, {NUM_HOPS}-hop)"
    ...
    print(f"  FINAL TEST RESULTS"
          f" (DistilBERT + ComplEx | {NUM_HOPS}-hop | 3 silos)")
    print("=" * 60)
