...
#
# Identical to Client3 PQ2H dataset.py in logic.
# The only structural difference is that build_neighbor_index is called with
# 5 silo KB paths instead of 3 — the function signature already accepts a list,
# so no code change is needed here.
#
# KB format    : tab-separated  (h\tr\tt)
# QA format    : question\ttopic_entity\tanswer  (3 columns, single answer)
...
def build_neighbor_index(kb_paths, shared_entity2id, max_neighbors=100):
    """
    Bidirectional neighbor index over ALL silo KBs.
    Accepts a list of paths — works for any number of silos (3 or 5).

    Forward + reverse edges give ~90% answer coverage for person-topic questions.
    """
...
def parse_qa_file(qa_path):
    """
    Parse PQ2H QA file.
    Format: question\ttopic_entity\tanswer  (tab-separated, 3 columns)
