# server.py — Federated server for WebQSP Client5 BERT+TransE (5 silos)
#
# Client5 has 5 silos → joint_dim = 5 * entity_dim = 5 * 256 = 1280
# (Client3 had 3 silos → joint_dim = 3 * 256 = 768)
#
# MLP: 768 → 512 → 1280  (output differs from Client3's 768)
#
...
class QuestionEncoder(nn.Module):
    """
    BERT [CLS] → MLP → q_embed in R^1280. BERT frozen.
    MLP: 768 → 512 → 1280.
    token_type_ids are passed (BERT uses segment embeddings).
    """
...
class FedVServer(nn.Module):
    """Central server — WebQSP Client5 BERT+TransE (5 silos)."""
