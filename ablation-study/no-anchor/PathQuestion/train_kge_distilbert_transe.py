# train_kge_distilbert_transe.py — Train TransE independently on each of the 3 silos (PQ2H)
...
class KGELogger:
    def __init__(self, log_dir, config_dict):
        ...
        with open(self.txt_path, "w") as f:
            f.write("=" * 70 + "\n")
            f.write("  FedV-KGQA  —  KGE Training Log  [TransE + DistilBERT | PQ2H]\n")
...
def train_silo(silo_name, kb_path, shared_entity2id, device, logger):
    print(f"\n{'='*60}")
    print(f"  Training TransE — {silo_name}  [for DistilBERT+TransE | PQ2H]")
    print(f"{'='*60}")
