# server.py — Local-Only baseline server
# Single silo's embeddings used directly. No fusion.
...
class QuestionEncoder(nn.Module):
    ...
    def __init__(self):
        ...
        for h_dim in MLP_HIDDEN_DIMS:
            ...
            in_dim = h_dim
        layers.append(nn.Linear(in_dim, JOINT_DIM))   # → 256
...
class LocalServer(nn.Module):
    """
    Local-only server — NO federation.
    Uses a single silo's entity embeddings directly.
    """
    ...
    def score_candidates(self, q_embed, h_silo, candidate_ids):
        safe_ids = candidate_ids.clamp(min=0)
        h_cands  = h_silo[safe_ids]
    ...
    def forward(self, questions, topic_ids, h_silo,
                answer_ids_batch, candidate_ids, device, margin=1.0):
        q_embed = self.question_encoder(questions, device)

        if USE_TOPIC_ANCHORING:
            q_final = q_embed + h_silo[topic_ids]
        else:
            q_final = q_embed

        candidate_ids = candidate_ids.to(device)
        sim  = self.score_candidates(q_final, h_silo, candidate_ids)
