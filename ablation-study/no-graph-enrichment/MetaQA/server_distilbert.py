# src/server_distilbert.py — Federated server for DistilBERT + TransE experiment
#
# Only difference from server.py (BERT version):
...
#   - Output hidden size is still 768, so MLP dims and h_joint dim are unchanged
...
class FedVServer(nn.Module):
    """
    Central server for DistilBERT + TransE FedV-KGQA (3 silos).
